...
function refreshBasketFunnel(basket) {
  ...
  let row = 1;
  sheet.getRange(row,1,1,14).merge().setValue(`${meta.emoji}  ${meta.label.toUpperCase()} — Stock Funnel`).setBackground(meta.color).setFontColor(COLORS.WHITE).setFontSize(15).setFontWeight('bold').setHorizontalAlignment('left');
  sheet.setRowHeight(row, 34); row++;
  sheet.getRange(row,1,1,14).merge().setValue(`Universe: 208 F&O · Target: ${config.target||'S1'} · Last refresh: ${nowIST()}`).setBackground(COLORS.SUBHEADER).setFontColor(COLORS.MUTED_LIGHT).setFontSize(9).setFontStyle('italic');
  sheet.setRowHeight(row, 20); row += 2;

  const filters = config.filters; const ncol = filters.length;
  sheet.getRange(row,1).setValue('Filter').setFontWeight('bold').setBackground(COLORS.DARK_HEADER).setFontColor(COLORS.WHITE);
  filters.forEach((f,i) => { sheet.getRange(row,2+i).setValue(f.metric).setFontWeight('bold').setBackground(COLORS.DARK_HEADER).setFontColor(COLORS.WHITE).setHorizontalAlignment('center').setWrap(true); });
  ...
  filters.forEach((f,i) => { sheet.getRange(row,2+i).setValue(f.min_display).setBackground(COLORS.SUBHEADER).setFontColor(COLORS.WHITE).setFontFamily(FONTS.MONO.family).setHorizontalAlignment('center'); }); row++;
  ...
  filters.forEach((f,i) => { sheet.getRange(row,2+i).setValue(f.max_display).setBackground(COLORS.SUBHEADER).setFontColor(COLORS.WHITE).setFontFamily(FONTS.MONO.family).setHorizontalAlignment('center'); }); row += 2;
  ...
  funnelCounts.forEach((c,i) => { sheet.getRange(row,2+i).setValue(c).setFontWeight('bold').setBackground(meta.color).setFontColor(COLORS.WHITE).setHorizontalAlignment('center').setFontSize(13); });
  sheet.setRowHeight(row, 30); row++;
  ...
  sheet.getRange(row,2,1,ncol).merge().setValue(`${qualified.count||0} stocks passed all filters`).setFontWeight('bold').setBackground(COLORS.DARK_HEADER).setFontColor(COLORS.WHITE).setHorizontalAlignment('center').setFontSize(12);
  ...
  sheet.getRange(row,1,1,ncol+1).setBackground(COLORS.DARK_HEADER).setFontColor(COLORS.WHITE); row++;
  row = renderTableHeader(sheet, row, SIGNAL_COLS, SIGNAL_COLS.length);
  ...
  sheet.setColumnWidth(1,140); sheet.setColumnWidths(2,SIGNAL_COLS.length,95); sheet.setFrozenRows(3);
