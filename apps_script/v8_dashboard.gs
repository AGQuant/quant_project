...
function scheduledRefresh() {
  ...
  const day = ist.getDay(); const hour = ist.getHours(); const min = ist.getMinutes();
...
function cleanRebuild() {
  ...
  const confirm = ui.alert('🧹 Clean Rebuild', 'This will DELETE all 10 Scorr tabs and rebuild them fresh from Railway.\n\nContinue?', ui.ButtonSet.YES_NO);
  ...
  Object.values(SHEETS).forEach(name => { const s = ss.getSheetByName(name); if (s) ss.deleteSheet(s); });
...
function pullLatestFromGitHub() {
  ...
  try {
    ...
    if (response.getResponseCode() !== 200) { SpreadsheetApp.getUi().alert('❌ GitHub fetch failed: HTTP ' + response.getResponseCode()); return; }
  ...
  } catch (e) { SpreadsheetApp.getUi().alert('❌ Could not reach GitHub:\n' + e); return; }

  if (remoteVersion === SCRIPT_VERSION) {
    SpreadsheetApp.getUi().alert('✅ Up to date', `You're on the latest version.\n\nCurrent: ${SCRIPT_VERSION}\nRemote: ${remoteVersion}`, SpreadsheetApp.getUi().ButtonSet.OK);
    return;
  }
  ...
  const html = HtmlService.createHtmlOutput(
    `<!DOCTYPE html><html><head><style>
      ...
      button:hover{background:#1D4ED8}ol{padding-left:20px;font-size:13px;line-height:1.6;color:#374151}
      ...
      <script>
        function copyUrl(){const text=${JSON.stringify(SCRIPT_RAW_URL)};
          navigator.clipboard.writeText(text).then(()=>{document.getElementById('btn').innerText='✓ Copied to clipboard'},()=>{
...
function showVersion() {
  const html = HtmlService.createHtmlOutput(
    `<!DOCTYPE html><html><head><style>
      body{font-family:-apple-system,sans-serif;padding:20px;color:#1F2937}h2{color:#9333EA;margin-top:0}
      .row{padding:6px 0}.label{color:#6B7280;display:inline-block;width:120px}.val{font-family:'Roboto Mono',monospace;font-weight:bold}
    </style></head><body>
      <h2>🟣 Scorr V8</h2>
...
function refreshBuyBaskets()  { toast('Refreshing buy baskets…');  ['buy_reversal','buy_momentum'].forEach(refreshBasketFunnel);   toast('✓ Buy baskets refreshed'); }
...
function fetchJSON(endpoint) {
  try {
    const response = UrlFetchApp.fetch(BASE_URL + endpoint, { muteHttpExceptions: true, headers: { 'Accept': 'application/json' } });
    const code = response.getResponseCode();
    if (code !== 200) { Logger.log(`API ${endpoint} returned ${code}: ${response.getContentText().slice(0,200)}`); return null; }
    return JSON.parse(response.getContentText());
  } catch (e) { Logger.log(`fetchJSON ${endpoint} failed: ${e}`); return null; }
}
...
function renderTodaysSignals(sheet, row) {
  ...
  BASKETS.forEach(basket => {
    const data = basket === 'sell_overbought' ? fetchSellOverbought() : fetchQualified(basket);
  ...
  BASKETS.forEach(basket => {
    ...
    sheet.getRange(row, 1, 1, SIGNAL_COLS.length).merge()
      ...
      .setBackground(meta.color).setFontColor(COLORS.WHITE).setFontSize(10).setFontWeight('bold').setHorizontalAlignment('left');
    sheet.setRowHeight(row, 22); row++;
    if (stocks.length === 0) {
      sheet.getRange(row, 1, 1, SIGNAL_COLS.length).merge().setValue('No signals today')
      ...
      stocks.forEach(s => {
        const vals = [s.symbol, fmtNum(s.gvm_score,2), fmtNum(s.dma_200,2), fmtNum(s.dma_50,2), fmtNum(s.rsi_month,1), fmtNum(s.rsi_weekly,1), fmtNum(s.month_return,2), fmtNum(s.week_return,2)];
        vals.forEach((v, i) => {
          sheet.getRange(row, 1+i).setValue(v).setFontFamily(i===0?FONTS.HEADER.family:FONTS.MONO.family).setFontWeight(i===0?'bold':'normal')
            .setFontColor(i===0?meta.color:null).setHorizontalAlignment(i===0?'left':'right').setBackground(row%2===0?COLORS.ALT_ROW:COLORS.CARD_BG);
        });
...
function renderTitleBar(sheet, row, title, subtitle) {
  sheet.getRange(row,1,1,10).merge().setValue(title).setBackground(COLORS.DARK_HEADER).setFontColor(COLORS.WHITE).setFontSize(16).setFontWeight('bold').setVerticalAlignment('middle').setHorizontalAlignment('left');
  sheet.setRowHeight(row, 38); row++;
  sheet.getRange(row,1,1,10).merge().setValue(`${subtitle}   ·   v${SCRIPT_VERSION}   ·   Last refresh: ${nowIST()}`).setBackground(COLORS.SUBHEADER).setFontColor(COLORS.MUTED_LIGHT).setFontSize(9).setFontStyle('italic').setHorizontalAlignment('left');
  sheet.setRowHeight(row, 22);
  return row + 1;
}

function renderSectionHeader(sheet, row, label, bg) {
  sheet.getRange(row,1,1,10).merge().setValue(label).setBackground(bg).setFontColor(COLORS.WHITE).setFontSize(11).setFontWeight('bold').setVerticalAlignment('middle').setHorizontalAlignment('left');
...
function renderPerformanceSummary(sheet, row) {
  ...
  STRATEGY_ORDER.forEach(strat => {
    ...
    row = renderDataRow(sheet, row, 8, [strat,dir,a.count,a.pnl,fmtPct(a.accuracy),a.winning,a.losing,a.avgPnl], {pnlCols:[4,8]});
  });
  ...
  STRATEGY_ORDER.forEach(strat => {
    ...
    if (strat === 'Manual/Untagged' && a.count === 0) return;
    const dir = strat.startsWith('Buy') ? 'LONG' : (strat.startsWith('Sell') ? 'SHORT' : 'MIXED');
    row = renderDataRow(sheet, row, 8, [strat,dir,a.count,a.pnl,fmtPct(a.accuracy),a.targetHit,a.slGap,a.avgPnl], {pnlCols:[4,8]});
  });
...
function renderMarketGate(sheet, row) {
  ...
  if (!mood) {
    sheet.getRange(row,1,1,10).merge().setValue('⚠ Could not fetch market mood — API unreachable').setBackground(COLORS.FAIL_BG).setFontColor(COLORS.FAIL_TEXT);
    return row + 1;
  }
  ...
  const buyBg = buyGateOpen?COLORS.PASS_BG:COLORS.FAIL_BG; const buyFg = buyGateOpen?COLORS.PASS_TEXT:COLORS.FAIL_TEXT;
  sheet.getRange(row,1,1,2).merge().setValue(buyGateOpen?'✅ BUY GATE OPEN':'🔒 BUY GATE CLOSED').setBackground(buyBg).setFontColor(buyFg).setFontSize(12).setFontWeight('bold').setHorizontalAlignment('center');
  sheet.getRange(row,3).setValue('Used').setFontWeight('bold').setBackground(COLORS.NEUTRAL_BG).setHorizontalAlignment('right');
  sheet.getRange(row,4).setValue(buyOpen).setFontFamily(FONTS.MONO.family).setFontSize(13).setFontWeight('bold').setHorizontalAlignment('center').setBackground(COLORS.BUY_REV).setFontColor(COLORS.WHITE);
  sheet.getRange(row,5).setValue('Available').setFontWeight('bold').setBackground(COLORS.NEUTRAL_BG).setHorizontalAlignment('right');
  sheet.getRange(row,6).setValue(`${buyRemaining} / ${buySlots}`).setFontFamily(FONTS.MONO.family).setFontSize(13).setFontWeight('bold').setHorizontalAlignment('center').setBackground(buyBg).setFontColor(buyFg);
  sheet.setRowHeight(row, 30); row++;

  const sellBg = sellGateOpen?COLORS.PASS_BG:COLORS.FAIL_BG; const sellFg = sellGateOpen?COLORS.PASS_TEXT:COLORS.FAIL_TEXT;
  sheet.getRange(row,1,1,2).merge().setValue(sellGateOpen?'✅ SELL GATE OPEN':'🔒 SELL GATE CLOSED').setBackground(sellBg).setFontColor(sellFg).setFontSize(12).setFontWeight('bold').setHorizontalAlignment('center');
  sheet.getRange(row,3).setValue('Used').setFontWeight('bold').setBackground(COLORS.NEUTRAL_BG).setHorizontalAlignment('right');
  sheet.getRange(row,4).setValue(sellOpen).setFontFamily(FONTS.MONO.family).setFontSize(13).setFontWeight('bold').setHorizontalAlignment('center').setBackground(COLORS.SELL_REV).setFontColor(COLORS.WHITE);
  sheet.getRange(row,5).setValue('Available').setFontWeight('bold').setBackground(COLORS.NEUTRAL_BG).setHorizontalAlignment('right');
  sheet.getRange(row,6).setValue(`${sellRemaining} / ${sellSlots}`).setFontFamily(FONTS.MONO.family).setFontSize(13).setFontWeight('bold').setHorizontalAlignment('center').setBackground(sellBg).setFontColor(sellFg);
  sheet.setRowHeight(row, 30); row++;
  return row;
}
...
function refreshBasketFunnel(basket) {
  ...
  if (!config || !qualified) { sheet.getRange(1,1).setValue('⚠ API unreachable'); return; }

  let row = 1;
  sheet.getRange(row,1,1,14).merge().setValue(`${meta.emoji}  ${meta.label.toUpperCase()} — Filter Funnel`).setBackground(meta.color).setFontColor(COLORS.WHITE).setFontSize(15).setFontWeight('bold').setHorizontalAlignment('left');
  sheet.setRowHeight(row, 34); row++;
  sheet.getRange(row,1,1,14).merge().setValue(`Universe: 208 F&O · Target: ${config.target||'S1'} · Win%: ${config.win_pct||'—'} · Refreshed: ${nowIST()}`).setBackground(COLORS.SUBHEADER).setFontColor(COLORS.MUTED_LIGHT).setFontSize(9).setFontStyle('italic');
  sheet.setRowHeight(row, 20); row += 2;

  const filters = config.filters;
  const ncol = filters.length;
  const funnelData = computeFunnelWithStocks(metricsAll, filters);
  const stages = funnelData.stages;
  const universeCount = funnelData.universe;

  // ── TC DROP % row (V4-style: shows how much each filter narrows the universe)
  sheet.getRange(row,1).setValue('TC DROP %').setFontWeight('bold').setBackground(COLORS.DARK_HEADER).setFontColor(COLORS.MUTED_LIGHT).setHorizontalAlignment('right').setFontSize(9);
  filters.forEach((f, i) => {
    const prev = i === 0 ? universeCount : stages[i-1].count;
    const curr = stages[i].count;
    const drop = prev === 0 ? 0 : Math.round(((prev - curr) / prev) * 100);
    const txt = drop === 0 ? '—' : `-${drop}%`;
    sheet.getRange(row, 2+i).setValue(txt)
      .setFontFamily(FONTS.MONO.family).setFontSize(9)
      .setFontWeight(drop > 30 ? 'bold' : 'normal')
      .setFontColor(drop > 40 ? COLORS.LOSS : drop > 15 ? '#EA580C' : COLORS.MUTED_LIGHT)
      .setHorizontalAlignment('center').setBackground(COLORS.DARK_HEADER);
  });
  sheet.setRowHeight(row, 22); row++;

  // ── Filter names
  sheet.getRange(row,1).setValue('Filter').setFontWeight('bold').setBackground(COLORS.SUBHEADER).setFontColor(COLORS.WHITE);
  filters.forEach((f,i) => {
    sheet.getRange(row,2+i).setValue(f.metric).setFontWeight('bold').setBackground(COLORS.SUBHEADER).setFontColor(COLORS.WHITE).setHorizontalAlignment('center').setWrap(true);
  });
  sheet.setRowHeight(row, 36); row++;

  // ── Min row
  sheet.getRange(row,1).setValue('Min').setFontStyle('italic').setBackground(COLORS.NEUTRAL_BG).setFontColor(COLORS.NEUTRAL_TEXT).setHorizontalAlignment('right');
  filters.forEach((f,i) => { sheet.getRange(row,2+i).setValue(f.min_display).setBackground(COLORS.NEUTRAL_BG).setFontColor(COLORS.NEUTRAL_TEXT).setFontFamily(FONTS.MONO.family).setHorizontalAlignment('center'); }); row++;

  // ── Max row
  sheet.getRange(row,1).setValue('Max').setFontStyle('italic').setBackground(COLORS.NEUTRAL_BG).setFontColor(COLORS.NEUTRAL_TEXT).setHorizontalAlignment('right');
  filters.forEach((f,i) => { sheet.getRange(row,2+i).setValue(f.max_display).setBackground(COLORS.NEUTRAL_BG).setFontColor(COLORS.NEUTRAL_TEXT).setFontFamily(FONTS.MONO.family).setHorizontalAlignment('center'); }); row++;
  ...
  stages.forEach((st, i) => {
    sheet.getRange(row,2+i).setValue(st.count).setFontWeight('bold').setBackground(meta.color).setFontColor(COLORS.WHITE).setHorizontalAlignment('center').setFontSize(13);
  });
  sheet.setRowHeight(row, 30); row++;
  ...
  sheet.getRange(row,2,1,ncol).merge().setValue(`${qualified.count||0} stocks passed all ${ncol} filters`).setFontWeight('bold').setBackground(COLORS.DARK_HEADER).setFontColor(COLORS.WHITE).setHorizontalAlignment('center').setFontSize(12);
  sheet.setRowHeight(row, 28); row += 2;

  // ── Stock matrix by stage (V4-style: each column = one filter stage, stocks listed below)
  sheet.getRange(row,1,1,ncol+1).merge().setValue('▼ STOCKS AT EACH FILTER STAGE').setFontWeight('bold').setBackground(COLORS.DARK_HEADER).setFontColor(COLORS.WHITE).setHorizontalAlignment('left');
  sheet.setRowHeight(row, 24); row++;

  // Sub-header: filter names + count as column headers
  ...
  filters.forEach((f,i) => {
    const cnt = stages[i].count;
    sheet.getRange(row,2+i).setValue(`${f.metric} (${cnt})`).setFontWeight('bold').setBackground(COLORS.NEUTRAL_BG).setFontColor(COLORS.NEUTRAL_TEXT).setFontSize(8).setHorizontalAlignment('center').setWrap(true);
  });
  sheet.setRowHeight(row, 28); row++;

  const maxStocks = Math.max(1, ...stages.map(s => s.symbols.length));
  const matrixRows = Math.min(maxStocks, 40);

  for (let r = 0; r < matrixRows; r++) {
    const rowBg = r % 2 === 0 ? COLORS.CARD_BG : COLORS.ALT_ROW;
    sheet.getRange(row+r, 1).setValue(r+1).setFontFamily(FONTS.MONO.family).setFontSize(8).setFontColor(COLORS.NEUTRAL_TEXT).setHorizontalAlignment('center').setBackground(rowBg);
    stages.forEach((st, i) => {
      const sym = st.symbols[r] || '';
      const isQualified = (i === stages.length - 1 && sym !== '');
      const cell = sheet.getRange(row+r, 2+i);
      cell.setValue(sym)
        .setFontFamily(FONTS.MONO.family).setFontSize(sym ? 9 : 8)
        .setFontWeight(isQualified ? 'bold' : 'normal')
        .setFontColor(isQualified ? meta.color : (sym ? COLORS.NEUTRAL_TEXT : COLORS.NEUTRAL_BG))
        .setHorizontalAlignment('center').setBackground(rowBg);
    });
    sheet.setRowHeight(row+r, 17);
  }
  row += matrixRows + 2;

  // ── Qualified stocks list
  sheet.getRange(row,1,1,SIGNAL_COLS.length).merge().setValue('▼ QUALIFIED STOCKS').setFontWeight('bold').setBackground(COLORS.DARK_HEADER).setFontColor(COLORS.WHITE);
  sheet.setRowHeight(row, 24); row++;
  row = renderTableHeader(sheet, row, SIGNAL_COLS, SIGNAL_COLS.length);

  const stocks = qualified.stocks || [];
  if (stocks.length === 0) {
    sheet.getRange(row,1,1,SIGNAL_COLS.length).merge().setValue('No qualified stocks today').setFontStyle('italic').setFontColor(COLORS.NEUTRAL_TEXT).setHorizontalAlignment('center').setBackground(COLORS.NEUTRAL_BG); row++;
  } else {
    stocks.forEach(s => {
      const vals = [s.symbol,fmtNum(s.gvm_score,2),fmtNum(s.dma_200,2),fmtNum(s.dma_50,2),fmtNum(s.rsi_month,1),fmtNum(s.rsi_weekly,1),fmtNum(s.month_return,2),fmtNum(s.week_return,2)];
      vals.forEach((v,i) => {
        sheet.getRange(row,1+i).setValue(v).setFontFamily(i===0?FONTS.HEADER.family:FONTS.MONO.family).setFontWeight(i===0?'bold':'normal').setHorizontalAlignment(i===0?'left':'right').setBackground(row%2===0?COLORS.ALT_ROW:COLORS.CARD_BG);
      });
      sheet.getRange(row,1,1,SIGNAL_COLS.length).setBorder(true,true,true,true,false,false,COLORS.BORDER_SOFT,SpreadsheetApp.BorderStyle.SOLID); row++;
    });
  }
  sheet.setColumnWidth(1, 90);
  sheet.setColumnWidths(2, Math.min(ncol+1, 14), 78);
  sheet.setFrozenRows(3);
  toast(`✓ ${meta.label} refreshed`);
}

function computeFunnelWithStocks(metricsAll, filters) {
  const stages = [];
  let pool = metricsAll.slice();
  const universe = pool.length;
  filters.forEach(f => {
    pool = pool.filter(stock => {
    ...
    stages.push({ count: pool.length, symbols: pool.map(s => s.symbol) });
  });
  return { stages, universe };
}

function computeFunnelCounts(metricsAll, filters) {
  return computeFunnelWithStocks(metricsAll, filters).stages.map(s => s.count);
}
...
function refreshSellOverbought() {
  ...
  let row = 1;
  sheet.getRange(row,1,1,14).merge().setValue(`${meta.emoji}  SELL OVERBOUGHT — Failed Breakout / Exhaustion Reversal`).setBackground(meta.color).setFontColor(COLORS.WHITE).setFontSize(15).setFontWeight('bold');
  sheet.setRowHeight(row, 34); row++;
  if (!data) { sheet.getRange(row,1).setValue('⚠ API unreachable'); return; }
  sheet.getRange(row,1,1,14).merge().setValue(`Target: ${data.target||'S1'} · SL: ${data.sl||'1:1'} · Backtest May-26: ${data.win_pct_may2026||'71.4%'} · Refreshed: ${nowIST()}`).setBackground(COLORS.SUBHEADER).setFontColor(COLORS.MUTED_LIGHT).setFontSize(9).setFontStyle('italic');
  sheet.setRowHeight(row, 20); row++;
  sheet.getRange(row,1,1,14).merge().setValue('ℹ ' + (data.note||'Market gate required — fails in recovery/bull markets')).setBackground('#FEF3C7').setFontColor('#92400E').setFontStyle('italic').setFontSize(10);
  sheet.setRowHeight(row, 24); row += 2;

  row = renderSubHeader(sheet, row, 'FILTER LOGIC');
  const filterRows = [['DMA 200','≥ 10%','Extended above 200-day MA'],['52-week index','≥ 80','Near 52-week high'],['MA9 vs MA21','≥ 3%','Short-term momentum stretched'],['Volume ratio','≤ 0.8','Volume drying — distribution starting'],['Range 1D','< 0','Today red — reversal trigger'],['RSI Month','≥ 60','RSI elevated']];
  ...
  filterRows.forEach(r => {
    ...
    sheet.getRange(row,1,1,3).setBorder(true,true,true,true,false,false,COLORS.BORDER_SOFT,SpreadsheetApp.BorderStyle.SOLID); row++;
  });
  row += 2;
  ...
  if (stocks.length === 0) {
    sheet.getRange(row,1,1,headers.length).merge().setValue('No qualified Sell Overbought signals today').setFontStyle('italic').setFontColor(COLORS.NEUTRAL_TEXT).setHorizontalAlignment('center').setBackground(COLORS.NEUTRAL_BG);
  } else {
    stocks.forEach(s => {
      const vals = [s.symbol,fmtNum(s.entry,2),fmtNum(s.target,2),fmtNum(s.stop,2),fmtPct(s.tgt_pct),fmtPct(s.dma_200),fmtNum(s.week_index_52,1),fmtPct(s.ma9_vs_ma21),fmtNum(s.vol_ratio,2),fmtNum(s.rsi_month,1),fmtPct(s.sector_week)];
      vals.forEach((v,i) => {
        sheet.getRange(row,1+i).setValue(v).setFontFamily(i===0?FONTS.HEADER.family:FONTS.MONO.family).setFontWeight(i===0?'bold':'normal').setFontColor(i===0?meta.color:null).setHorizontalAlignment(i===0?'left':'right').setBackground(row%2===0?COLORS.ALT_ROW:COLORS.CARD_BG);
      });
      sheet.getRange(row,1,1,headers.length).setBorder(true,true,true,true,false,false,COLORS.BORDER_SOFT,SpreadsheetApp.BorderStyle.SOLID); row++;
    });
  }
  sheet.setColumnWidth(1,140); sheet.setColumnWidths(2,headers.length-1,100); sheet.setFrozenRows(2);
...
function fetchCmp(symbol)    { const r = fetchJSON('/api/cmp/' + encodeURIComponent(symbol)); return (r && r.cmp !== undefined && r.cmp !== null) ? Number(r.cmp) : null; }
...
function paperHolding(entryTs, exitTs) {
  if (!entryTs) return '—';
  try {
    const a = new Date(entryTs); const b = exitTs ? new Date(exitTs) : new Date();
    const days = Math.floor((b-a)/86400000);
    if (days <= 0) return 'Intraday'; if (days === 1) return '1 Day'; return days + ' Days';
  } catch (e) { return '—'; }
}
...
function refreshInPosition() {
  ...
  let row = 1;
  sheet.getRange(row,1,1,11).merge().setValue('📍  IN POSITION — PAPER ENGINE (Live Open Positions)').setBackground(COLORS.DARK_HEADER).setFontColor(COLORS.WHITE).setFontSize(15).setFontWeight('bold');
  sheet.setRowHeight(row, 34); row++;
  ...
  [['Total Open',positions.length,'Long Open',longs.length,'Short Open',shorts.length],['Total P&L',fmtPnL(totPnl),'Long P&L',fmtPnL(longPnl),'Short P&L',fmtPnL(shortPnl)]].forEach(srow => {
    for (let i = 0; i < srow.length; i++) {
      const cell = sheet.getRange(row,1+i);
      i%2===0 ? cell.setValue(srow[i]).setFontWeight('bold').setBackground(COLORS.NEUTRAL_BG).setHorizontalAlignment('right') : cell.setValue(srow[i]).setFontFamily(FONTS.MONO.family).setFontWeight('bold').setHorizontalAlignment('center');
    }
    row++;
  });
  sheet.setColumnWidth(1,130); sheet.setColumnWidths(2,10,105); sheet.setFrozenRows(2);
  toast('✓ In Position (paper) refreshed');
}

function paperUnrealised(p, cmpMap) {
  const entry = Number(p.entry_price||0); const qty = Number(p.qty||0);
...
function sumPaperUnrealised(arr, cmpMap) {
  return arr.reduce((s,p) => { const v = paperUnrealised(p,cmpMap); return s+(v===null?0:v); }, 0);
}

function renderPaperPositionSection(sheet, row, strategy, positions, cmpMap) {
  const meta = strategyMeta(strategy);
  sheet.getRange(row,1,1,11).merge().setValue(`${meta.emoji}  ${strategy.toUpperCase()}`).setBackground(meta.color).setFontColor(COLORS.WHITE).setFontSize(11).setFontWeight('bold');
  sheet.setRowHeight(row, 26); row++;

  let winning=0,losing=0,totPnl=0,priced=0;
  positions.forEach(p => { const v=paperUnrealised(p,cmpMap); if(v!==null){totPnl+=v;priced++;if(v>0)winning++;else if(v<0)losing++;} });
  ...
  if (positions.length === 0) {
    sheet.getRange(row,1,1,headers.length).merge().setValue('No open paper positions').setFontStyle('italic').setFontColor(COLORS.NEUTRAL_TEXT).setHorizontalAlignment('center').setBackground(COLORS.NEUTRAL_BG); row++;
  } else {
    positions.forEach(p => {
      const pnl = paperUnrealised(p,cmpMap);
      const cmp = cmpMap&&cmpMap[p.symbol]!=null?cmpMap[p.symbol]:null;
      const vals = [fmtDate(p.entry_ts),p.symbol,paperIsLong(p.side)?'LONG':'SHORT',fmtNum(p.entry_price,2),cmp===null?'—':fmtNum(cmp,2),p.qty,fmtNum(p.stop_loss,2),fmtNum(p.target,2),pnl===null?'—':fmtPnL(pnl),paperHolding(p.entry_ts),p.pivot_date||'—'];
      vals.forEach((v,i) => {
        const cell = sheet.getRange(row,1+i);
        cell.setValue(v).setFontFamily(i===1?FONTS.HEADER.family:FONTS.MONO.family).setFontWeight(i===1?'bold':'normal').setHorizontalAlignment(i===1?'left':'right').setBackground(row%2===0?COLORS.ALT_ROW:COLORS.CARD_BG);
...
function refreshTradeLog() {
  ...
  let row = 1;
  sheet.getRange(row,1,1,10).merge().setValue('📒  TRADE LOG — PAPER ENGINE (Closed Trades)').setBackground(COLORS.DARK_HEADER).setFontColor(COLORS.WHITE).setFontSize(15).setFontWeight('bold');
  sheet.setRowHeight(row, 34); row++;
  sheet.getRange(row,1,1,10).merge().setValue(`Last refresh: ${nowIST()}`).setBackground(COLORS.SUBHEADER).setFontColor(COLORS.MUTED_LIGHT).setFontSize(9).setFontStyle('italic');
  sheet.setRowHeight(row, 20); row += 2;
  ...
  STRATEGY_ORDER.forEach(strat => {
    const list = grouped[strat] || [];
    if (strat === 'Manual/Untagged' && list.length === 0) return;
    row = renderPaperTradeSection(sheet, row, strat, list); row += 1;
  });
  ...
  [['Total Closed',trN,'Accuracy',fmtPct(acc)],['Wins',wins,'Losses',los],['Gate Exits',gate,'Gap Exits',gap],['Total P&L',fmtPnL(tot),'Avg Return',fmtPct(avgR)]].forEach(srow => {
    for (let i=0;i<srow.length;i++) {
      const cell = sheet.getRange(row,1+i);
      i%2===0?cell.setValue(srow[i]).setFontWeight('bold').setBackground(COLORS.NEUTRAL_BG).setHorizontalAlignment('right'):cell.setValue(srow[i]).setFontFamily(FONTS.MONO.family).setFontWeight('bold').setHorizontalAlignment('center');
    }
  ...
  sheet.setColumnWidth(1,130); sheet.setColumnWidths(2,9,110); sheet.setFrozenRows(2);
  toast('✓ Trade Log (paper) refreshed');
}

function renderPaperTradeSection(sheet, row, strategy, trades) {
  const meta = strategyMeta(strategy);
  sheet.getRange(row,1,1,10).merge().setValue(`${meta.emoji}  ${strategy.toUpperCase()}`).setBackground(meta.color).setFontColor(COLORS.WHITE).setFontSize(11).setFontWeight('bold');
  sheet.setRowHeight(row, 26); row++;

  let pnlSum=0,targetHit=0,slGap=0;
  trades.forEach(t => { pnlSum+=Number(t.pnl||0); String(t.result||'').toLowerCase().includes('target')?targetHit++:slGap++; });
  ...
  row = renderTableHeader(sheet, row, headers, headers.length);
  if (trades.length === 0) {
    sheet.getRange(row,1,1,headers.length).merge().setValue('No closed paper trades for this strategy').setFontStyle('italic').setFontColor(COLORS.NEUTRAL_TEXT).setHorizontalAlignment('center').setBackground(COLORS.NEUTRAL_BG); row++;
  } else {
    trades.forEach(t => {
      const pnl = Number(t.pnl||0); const result = t.result||'Unknown';
      const vals = [fmtDate(t.entry_ts),fmtDate(t.exit_ts),t.symbol,paperIsLong(t.side)?'LONG':'SHORT',fmtNum(t.entry_price,2),fmtNum(t.exit_price,2),fmtPnL(pnl),fmtPct(t.return_pct),paperHolding(t.entry_ts,t.exit_ts),result];
      vals.forEach((v,i) => {
        const cell = sheet.getRange(row,1+i);
        cell.setValue(v).setFontFamily(i===2?FONTS.HEADER.family:FONTS.MONO.family).setFontWeight(i===2?'bold':'normal').setHorizontalAlignment(i===2?'left':'right').setBackground(row%2===0?COLORS.ALT_ROW:COLORS.CARD_BG);
        if(i===6) cell.setFontColor(pnl>=0?COLORS.PROFIT:COLORS.LOSS).setFontWeight('bold');
        if(i===9) { const isWin=String(result).toLowerCase().includes('target'); cell.setFontColor(isWin?COLORS.PASS_TEXT:COLORS.FAIL_TEXT).setBackground(isWin?COLORS.PASS_BG:COLORS.FAIL_BG).setFontWeight('bold').setHorizontalAlignment('center'); }
      });
      sheet.getRange(row,1,1,headers.length).setBorder(true,true,true,true,false,false,COLORS.BORDER_SOFT,SpreadsheetApp.BorderStyle.SOLID); row++;
    });
  }
...
function refreshRawData() {
  ...
  let row = 1; const ncol = RAW_COLS.length;
  sheet.getRange(row,1,1,ncol).merge().setValue('🗃️  RAW DATA — CMP + Pivots + 21 Metrics (all active futures)').setBackground(COLORS.DARK_HEADER).setFontColor(COLORS.WHITE).setFontSize(15).setFontWeight('bold');
  sheet.setRowHeight(row, 34); row++;
  if (!data || !data.stocks) { sheet.getRange(row,1).setValue('⚠ API unreachable — /api/v8/raw returned no data'); return; }

  const cmpMap = {}; const live = fetchLiveMetrics();
  if (Array.isArray(live)) live.forEach(r => { if(r&&r.symbol) cmpMap[r.symbol]=r.cmp; });
  const pivMap = {}; const piv = fetchPaperPivots();
  if (Array.isArray(piv)) piv.forEach(p => { if(p&&p.symbol) pivMap[p.symbol]=p; });

  sheet.getRange(row,1,1,ncol).merge().setValue(`${data.count||0} stocks · Score date: ${data.score_date||'—'} · CMP: ${Object.keys(cmpMap).length} · Pivots: ${Object.keys(pivMap).length} · GVM-sorted · Refreshed: ${nowIST()}`).setBackground(COLORS.SUBHEADER).setFontColor(COLORS.MUTED_LIGHT).setFontSize(9).setFontStyle('italic');
  sheet.setRowHeight(row, 20); row += 2;

  RAW_COLS.forEach((h,i) => {
    sheet.getRange(row,1+i).setValue(h).setFontWeight('bold').setBackground(COLORS.NEUTRAL_BG).setFontSize(9).setFontColor(COLORS.NEUTRAL_TEXT).setHorizontalAlignment('center').setWrap(true).setBorder(true,true,true,true,false,false,COLORS.BORDER_SOFT,SpreadsheetApp.BorderStyle.SOLID);
  });
  sheet.setRowHeight(row, 30); const headerRow = row; row++;

  const stocks = data.stocks || [];
  if (stocks.length === 0) {
    sheet.getRange(row,1,1,ncol).merge().setValue('No metrics rows for the latest score date').setFontStyle('italic').setFontColor(COLORS.NEUTRAL_TEXT).setHorizontalAlignment('center').setBackground(COLORS.NEUTRAL_BG); row++;
  } else {
    const matrix = stocks.map(s => {
      const sym=s.symbol; const cmp=cmpMap[sym]; const pv=pivMap[sym]||{};
      const head = [sym,(cmp===undefined||cmp===null)?'—':fmtNum(cmp,2),fmtNum(pv.pp,2),fmtNum(pv.r1,2),fmtNum(pv.s1,2),fmtNum(pv.r2,2),fmtNum(pv.s2,2)];
      const metrics = RAW_FIELDS.slice(1).map(f => {
        const v=s[f];
        if(f==='gvm_score') return fmtNum(v,2);
        if(f==='rsi_month'||f==='rsi_weekly'||f==='daily_rsi'||f==='month_index'||f==='week_index_52') return fmtNum(v,1);
        return fmtNum(v,2);
      });
      return head.concat(metrics);
    });
    const dataRange = sheet.getRange(row,1,matrix.length,ncol);
    dataRange.setValues(matrix); dataRange.setFontFamily(FONTS.MONO.family).setFontSize(9);
    sheet.getRange(row,1,matrix.length,1).setFontFamily(FONTS.HEADER.family).setFontWeight('bold').setHorizontalAlignment('left');
    sheet.getRange(row,2,matrix.length,ncol-1).setHorizontalAlignment('right');
    for (let r=0;r<matrix.length;r++) {
      sheet.getRange(row+r,1,1,ncol).setBackground(r%2===0?COLORS.CARD_BG:COLORS.ALT_ROW).setBorder(true,true,true,true,false,false,COLORS.BORDER_SOFT,SpreadsheetApp.BorderStyle.SOLID);
    }
    row += matrix.length;
  }
  sheet.setColumnWidth(1,130); sheet.setColumnWidth(2,82); sheet.setColumnWidths(3,5,78); sheet.setColumnWidths(8,ncol-7,74);
  sheet.setFrozenRows(headerRow); sheet.setFrozenColumns(2);
...
const SCAN_FILTER_ABBR = {
  ...
  rsi_month:'RSIm',rsi_weekly:'RSIw',daily_rsi:'RSId',month_return:'MthRet',week_return:'WkRet',
  sector_week:'SecWk',sector_day:'SecDay',month_index:'MthIdx',week_index_52:'wi52',
  range_1d:'Rng1D',range_3d:'Rng3D',ma9_vs_ma21:'ma9_21',vol_ratio:'VolR',
};

function scanPassesFilter(value, f) {
  if (value===null||value===undefined||value==='') return false;
  const v=Number(value); if(isNaN(v)) return false;
  if(f.min!==null&&f.min!==undefined&&v<f.min) return false;
  if(f.max!==null&&f.max!==undefined&&v>f.max) return false;
  return true;
}
...
function scanStockAgainstBasket(stock, filters) {
  const passed = [];
  filters.forEach(f => { if(scanPassesFilter(stock[f.metric],f)) passed.push(SCAN_FILTER_ABBR[f.metric]||f.metric); });
  return {passed,count:passed.length,total:filters.length};
}
...
function refreshFilterScan() {
  ...
  sheet.clear().clearConditionalFormatRules();
  sheet.setHiddenGridlines(true);
  const data = fetchRawData(); const configs = {}; let configOk = true;
  BASKETS.forEach(b => { const c=fetchFilterConfig(b); if(!c||!c.filters) configOk=false; configs[b]=c; });

  let row=1; const NCOL=11;
  sheet.getRange(row,1,1,NCOL).merge().setValue('🔎  FILTER SCAN — Per-Stock Pass Count Across 5 Baskets').setBackground(COLORS.DARK_HEADER).setFontColor(COLORS.WHITE).setFontSize(15).setFontWeight('bold');
  sheet.setRowHeight(row, 34); row++;

  if (!data||!data.stocks||!configOk) {
    sheet.getRange(row,1,1,NCOL).merge().setValue('⚠ API unreachable — /api/v8/raw or a filter_config returned no data').setBackground(COLORS.FAIL_BG).setFontColor(COLORS.FAIL_TEXT); return;
  }
  const stocks=data.stocks||[]; const scoreDate=data.score_date||'—';
  sheet.getRange(row,1,1,NCOL).merge().setValue(`${stocks.length} stocks · Score date: ${scoreDate} · Count = passed/total · Green = full qualify · Refreshed: ${nowIST()}`).setBackground(COLORS.SUBHEADER).setFontColor(COLORS.MUTED_LIGHT).setFontSize(9).setFontStyle('italic');
  sheet.setRowHeight(row, 20); row += 2;

  const bandRow=row;
  sheet.getRange(bandRow,1).setValue('Symbol').setFontWeight('bold').setBackground(COLORS.DARK_HEADER).setFontColor(COLORS.WHITE).setHorizontalAlignment('center').setVerticalAlignment('middle');
  sheet.getRange(bandRow,1,2,1).merge();
  BASKETS.forEach((b,i) => {
    const meta=BASKET_META[b]; const startCol=2+i*2;
    sheet.getRange(bandRow,startCol,1,2).merge().setValue(`${meta.emoji}  ${meta.label}`).setBackground(meta.color).setFontColor(COLORS.WHITE).setFontWeight('bold').setFontSize(10).setHorizontalAlignment('center');
  });
  sheet.setRowHeight(bandRow, 24); row++;
  const subRow=row;
  BASKETS.forEach((b,i) => {
    const startCol=2+i*2;
    sheet.getRange(subRow,startCol).setValue('Cnt').setFontWeight('bold').setBackground(COLORS.NEUTRAL_BG).setFontColor(COLORS.NEUTRAL_TEXT).setFontSize(9).setHorizontalAlignment('center');
    sheet.getRange(subRow,startCol+1).setValue('Filters Passed').setFontWeight('bold').setBackground(COLORS.NEUTRAL_BG).setFontColor(COLORS.NEUTRAL_TEXT).setFontSize(9).setHorizontalAlignment('left');
  });
  sheet.setRowHeight(subRow, 20); const headerBottom=subRow; row++;

  const matrix=[];
  stocks.forEach(s => {
    const line=[s.symbol]; let bestRatio=0;
    BASKETS.forEach(b => {
      const res=scanStockAgainstBasket(s,configs[b].filters);
      line.push(`${res.count}/${res.total}`); line.push(res.passed.join(', '));
      const ratio=res.total>0?res.count/res.total:0; if(ratio>bestRatio) bestRatio=ratio;
    });
    line.push(bestRatio); matrix.push(line);
  });
  matrix.sort((a,b)=>b[NCOL]-a[NCOL]);
  matrix.forEach(line=>line.pop());

  if (matrix.length===0) {
    sheet.getRange(row,1,1,NCOL).merge().setValue('No stocks in universe for the latest score date').setFontStyle('italic').setFontColor(COLORS.NEUTRAL_TEXT).setHorizontalAlignment('center').setBackground(COLORS.NEUTRAL_BG); row++;
  } else {
    const dataRange=sheet.getRange(row,1,matrix.length,NCOL);
    dataRange.setValues(matrix); dataRange.setFontFamily(FONTS.MONO.family).setFontSize(9).setVerticalAlignment('middle');
    sheet.getRange(row,1,matrix.length,1).setFontFamily(FONTS.HEADER.family).setFontWeight('bold').setHorizontalAlignment('left');
    BASKETS.forEach((b,i) => {
      const cntCol=2+i*2;
    ...
    for(let r=0;r<matrix.length;r++) {
      BASKETS.forEach((b,i) => {
        const cntCol=2+i*2; const cellVal=matrix[r][cntCol-1]; const parts=String(cellVal).split('/');
        if(parts.length===2&&parts[0]===parts[1]&&Number(parts[1])>0) {
          sheet.getRange(row+r,cntCol).setBackground(COLORS.PASS_BG).setFontColor(COLORS.PASS_TEXT).setFontWeight('bold');
        }
      });
    }
    row += matrix.length;
  ...
  BASKETS.forEach((b,i) => { const cntCol=2+i*2; sheet.setColumnWidth(cntCol,48); sheet.setColumnWidth(cntCol+1,230); });
  sheet.setFrozenRows(headerBottom); sheet.setFrozenColumns(1);
...
function aggregatePositions(positions) {
  ...
  positions.forEach(p => {
    const strat=inferStrategy(p); if(!out[strat]) out[strat]=zeroAgg();
    const pnl=computeUnrealisedPnl(p);
    out[strat].count++; out[strat].pnl+=pnl; if(pnl>0) out[strat].winning++; else if(pnl<0) out[strat].losing++;
    out.__TOTAL.count++; out.__TOTAL.pnl+=pnl; if(pnl>0) out.__TOTAL.winning++; else if(pnl<0) out.__TOTAL.losing++;
  });
...
function aggregateTrades(trades) {
  const out = {__TOTAL:zeroAgg()};
  trades.forEach(t => {
    const strat=inferStrategy(t); if(!out[strat]) out[strat]=zeroAgg();
    const pnl=computeClosedPnl(t); const result=(t.Result||t.result||'').toLowerCase();
...
function isLongTrade(row) {
  // v1.5.0 — also checks 'side' field used by paper engine positions/trades
  const d = String(row.Direction||row.direction||row.Type||row.type||row.side||'').toUpperCase();
...
function computeUnrealisedPnl(p) {
  ...
  const entry=Number(p.entry_price||p.entry||0); const cmp=Number(p.cmp||0); const qty=Number(p.qty||0);
...
function computeClosedPnl(t) {
  ...
  if(explicit!==undefined&&explicit!==null&&explicit!=='') return Number(explicit)||0;
  const entry=Number(t.entry||0); const exit=Number(t.exit||0); const qty=Number(t.qty||0);
...
function computePositionStats(trades) {
  ...
  trades.forEach(t=>{const pnl=computeUnrealisedPnl(t);s.totalPnl+=pnl;if(pnl>0)s.winning++;else if(pnl<0)s.losing++;});
...
function computeClosedStats(trades) {
  ...
  trades.forEach(t=>{
    s.pnl+=computeClosedPnl(t);const r=String(t.Result||t.result||'').toLowerCase();
    if(r.includes('target'))s.targetHit++;else if(r.includes('sl'))s.slHit++;else if(r.includes('gate'))s.gateExit++;else if(r.includes('gap'))s.gapExit++;
  });
...
function computeClosedStatsForGroup(trades) {
  ...
  trades.forEach(t=>{s.pnl+=computeClosedPnl(t);String(t.Result||t.result||'').toLowerCase().includes('target')?s.targetHit++:s.slGap++;});
  s.accuracy=s.count>0?(s.targetHit/s.count)*100:0;s.avgPnl=s.count>0?s.pnl/s.count:0;
  return s;
}

function computeHolding(entryTime,exitTime) {
  if(!entryTime) return '—';
  try{const entry=new Date(entryTime);const exit=exitTime?new Date(exitTime):new Date();const days=Math.floor((exit-entry)/86400000);if(days===0)return 'Intraday';if(days===1)return '1 Day';return days+' Days';}catch(e){return '—';}
}
...
function renderSubHeader(sheet, row, label) {
  sheet.getRange(row,1,1,10).merge().setValue(label).setBackground(COLORS.SUBHEADER).setFontColor(COLORS.WHITE).setFontSize(10).setFontWeight('bold').setHorizontalAlignment('left');
  sheet.setRowHeight(row, 24); return row+1;
}

function renderTableHeader(sheet, row, headers, ncol) {
  headers.forEach((h,i) => {
    sheet.getRange(row,1+i).setValue(h).setFontWeight('bold').setBackground(COLORS.NEUTRAL_BG).setFontSize(10).setFontColor(COLORS.NEUTRAL_TEXT).setHorizontalAlignment('center').setBorder(true,true,true,true,false,false,COLORS.BORDER_SOFT,SpreadsheetApp.BorderStyle.SOLID);
  });
  return row+1;
}
...
function renderDataRow(sheet, row, ncol, vals, opts) {
  ...
  vals.forEach((v,i) => {
    const cell=sheet.getRange(row,1+i);
    cell.setValue(v).setFontFamily(i===0?FONTS.HEADER.family:FONTS.MONO.family).setFontWeight(i===0?'bold':'normal').setHorizontalAlignment(i===0?'left':'center').setBackground(row%2===0?COLORS.ALT_ROW:COLORS.CARD_BG);
    if(opts.pnlCols&&opts.pnlCols.includes(i+1)) cell.setFontColor(parsePnL(v)>=0?COLORS.PROFIT:COLORS.LOSS).setFontWeight('bold');
  });
...
function renderTotalRow(sheet, row, ncol, vals, opts) {
  ...
  vals.forEach((v,i) => {
    const cell=sheet.getRange(row,1+i);
    cell.setValue(v).setFontFamily(i===0?FONTS.HEADER.family:FONTS.MONO.family).setFontWeight('bold').setHorizontalAlignment(i===0?'left':'center').setBackground(COLORS.DARK_HEADER).setFontColor(COLORS.WHITE).setFontSize(11);
    if(opts.pnlCols&&opts.pnlCols.includes(i+1)) cell.setFontColor(parsePnL(v)>=0?'#86EFAC':'#FCA5A5');
  });
  sheet.setRowHeight(row, 28); return row+1;
}

function fmtNum(v,decimals) { if(v===null||v===undefined||v==='') return '—'; const n=Number(v); if(isNaN(n)) return v; return n.toFixed(decimals===undefined?2:decimals); }
function fmtPct(v) { if(v===null||v===undefined||v===''||isNaN(Number(v))) return '—'; return Number(v).toFixed(2)+'%'; }
function fmtPnL(v) { if(v===null||v===undefined||v===''||isNaN(Number(v))) return '₹0'; const n=Number(v); return (n<0?'-':'')+'₹'+Math.abs(Math.round(n)).toLocaleString('en-IN'); }
function parsePnL(s) { if(typeof s==='number') return s; if(!s) return 0; const n=Number(String(s).replace(/[₹,]/g,'').replace(/[^\d.\-]/g,'')); return isNaN(n)?0:n; }
function fmtDate(d) { if(!d) return '—'; try{return Utilities.formatDate(new Date(d),'Asia/Kolkata','d MMM HH:mm');}catch(e){return d;} }
function nowIST() { return Utilities.formatDate(new Date(),'Asia/Kolkata','d-MMM-yyyy HH:mm:ss'); }
