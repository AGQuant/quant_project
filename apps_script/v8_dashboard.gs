      ...
      <script>
        function copyUrl() {
          ...
          navigator.clipboard.writeText(text).then(
            ...
            () => { const r = document.createRange(); r.selectNode(document.getElementById('url')); window.getSelection().removeAllRanges(); window.getSelection().addRange(r); document.execCommand('copy'); document.getElementById('btn').innerText = '✓ Copied (fallback)'; }
          );
        }
      </script>
    </body></html>`
...
function showVersion() {
  const html = HtmlService.createHtmlOutput(
    `<!DOCTYPE html><html><head><style>
      ...
      .row { padding: 6px 0; } .label { color: #6B7280; display: inline-block; width: 120px; }
...
function refreshAll() {
  ...
  refreshSellOverbought(); refreshInPosition(); refreshTradeLog(); refreshRawData(); refreshFilterScan();
...
function renderTodaysSignals(sheet, row) {
  ...
  BASKETS.forEach(basket => {
    ...
    if (stocks.length === 0) {
      sheet.getRange(row, 1, 1, SIGNAL_COLS.length).merge().setValue('No signals today').setFontStyle('italic').setFontColor(COLORS.NEUTRAL_TEXT).setHorizontalAlignment('center').setBackground(COLORS.NEUTRAL_BG);
      ...
      stocks.forEach(s => {
        ...
        vals.forEach((v, i) => {
          sheet.getRange(row, 1 + i).setValue(v)
            .setFontFamily(i === 0 ? FONTS.HEADER.family : FONTS.MONO.family).setFontWeight(i === 0 ? 'bold' : 'normal')
            .setFontColor(i === 0 ? meta.color : null).setHorizontalAlignment(i === 0 ? 'left' : 'right')
...
function renderTitleBar(sheet, row, title, subtitle) {
  ...
  sheet.getRange(row, 1, 1, 10).merge().setValue(stamp).setBackground(COLORS.SUBHEADER).setFontColor(COLORS.MUTED_LIGHT).setFontSize(9).setFontStyle('italic').setHorizontalAlignment('left');
...
function renderMarketGate(sheet, row) {
  ...
  // v1.5.0 — Count open buy/sell positions from PAPER engine (was personal_journal, now paper)
...
function renderFilterCard(sheet, row, basket) {
  ...
  sheet.getRange(row, 1, 1, 10).merge().setValue(`${meta.emoji}  ${meta.label.toUpperCase()}   ·   Target: ${config.target || 'S1'}   ·   Win%: ${config.win_pct || '—'}`).setBackground(meta.color).setFontColor(COLORS.WHITE).setFontSize(11).setFontWeight('bold').setHorizontalAlignment('left');
...
function refreshBasketFunnel(basket) {
  ...
  filters.forEach((f, i) => { sheet.getRange(row, 2 + i).setValue(f.min_display).setBackground(COLORS.SUBHEADER).setFontColor(COLORS.WHITE).setFontFamily(FONTS.MONO.family).setHorizontalAlignment('center'); });
  row++;
  ...
  filters.forEach((f, i) => { sheet.getRange(row, 2 + i).setValue(f.max_display).setBackground(COLORS.SUBHEADER).setFontColor(COLORS.WHITE).setFontFamily(FONTS.MONO.family).setHorizontalAlignment('center'); });
  row += 2;
  ...
  if (stocks.length === 0) {
    sheet.getRange(row, 1, 1, SIGNAL_COLS.length).merge().setValue('No qualified stocks today').setFontStyle('italic').setFontColor(COLORS.NEUTRAL_TEXT).setHorizontalAlignment('center').setBackground(COLORS.NEUTRAL_BG);
    row++;
  } else {
    stocks.forEach(s => {
      const vals = [s.symbol, fmtNum(s.gvm_score, 2), fmtNum(s.dma_200, 2), fmtNum(s.dma_50, 2), fmtNum(s.rsi_month, 1), fmtNum(s.rsi_weekly, 1), fmtNum(s.month_return, 2), fmtNum(s.week_return, 2)];
      vals.forEach((v, i) => { sheet.getRange(row, 1 + i).setValue(v).setFontFamily(i === 0 ? FONTS.HEADER.family : FONTS.MONO.family).setFontWeight(i === 0 ? 'bold' : 'normal').setHorizontalAlignment(i === 0 ? 'left' : 'right').setBackground(row % 2 === 0 ? COLORS.ALT_ROW : COLORS.CARD_BG); });
      sheet.getRange(row, 1, 1, SIGNAL_COLS.length).setBorder(true, true, true, true, false, false, COLORS.BORDER_SOFT, SpreadsheetApp.BorderStyle.SOLID);
      row++;
    });
  }
...
function refreshSellOverbought() {
  ...
  if (stocks.length === 0) {
    ...
    stocks.forEach(s => {
      ...
      vals.forEach((v, i) => { sheet.getRange(row, 1 + i).setValue(v).setFontFamily(i === 0 ? FONTS.HEADER.family : FONTS.MONO.family).setFontWeight(i === 0 ? 'bold' : 'normal').setFontColor(i === 0 ? meta.color : null).setHorizontalAlignment(i === 0 ? 'left' : 'right').setBackground(row % 2 === 0 ? COLORS.ALT_ROW : COLORS.CARD_BG); });
...
function sumPaperUnrealised(arr, cmpMap) { return arr.reduce((s, p) => { const v = paperUnrealised(p, cmpMap); return s + (v === null ? 0 : v); }, 0); }
...
function renderPaperPositionSection(sheet, row, strategy, positions, cmpMap) {
  ...
  ['Open', 'Unrealised P&L', 'Accuracy', 'Winning', 'Losing', 'Avg P&L/Trade'].forEach((h, i) => { sheet.getRange(row, 1 + i).setValue(h).setFontWeight('bold').setBackground(COLORS.NEUTRAL_BG).setFontSize(9).setFontColor(COLORS.NEUTRAL_TEXT).setHorizontalAlignment('center'); });
  row++;
...
function renderPaperTradeSection(sheet, row, strategy, trades) {
  ...
  ['Closed', 'Booked P&L', 'Accuracy', 'Target Hit', 'SL/Gate/Gap', 'Avg P&L/Trade'].forEach((h, i) => { sheet.getRange(row, 1 + i).setValue(h).setFontWeight('bold').setBackground(COLORS.NEUTRAL_BG).setFontSize(9).setFontColor(COLORS.NEUTRAL_TEXT).setHorizontalAlignment('center'); });
  row++;
...
function refreshRawData() {
  ...
  sheet.getRange(row, 1, 1, ncol).merge().setValue(`${data.count || 0} stocks · Score date: ${scoreDate} · CMP: ${Object.keys(cmpMap).length} · Pivots: ${Object.keys(pivMap).length} · GVM-sorted · Refreshed: ${nowIST()}`).setBackground(COLORS.SUBHEADER).setFontColor(COLORS.MUTED_LIGHT).setFontSize(9).setFontStyle('italic');
  sheet.setRowHeight(row, 20); row += 2;

  RAW_COLS.forEach((h, i) => { sheet.getRange(row, 1 + i).setValue(h).setFontWeight('bold').setBackground(COLORS.NEUTRAL_BG).setFontSize(9).setFontColor(COLORS.NEUTRAL_TEXT).setHorizontalAlignment('center').setWrap(true).setBorder(true, true, true, true, false, false, COLORS.BORDER_SOFT, SpreadsheetApp.BorderStyle.SOLID); });
  sheet.setRowHeight(row, 30);
  const headerRow = row; row++;

  const stocks = data.stocks || [];
  if (stocks.length === 0) {
    sheet.getRange(row, 1, 1, ncol).merge().setValue('No metrics rows for the latest score date').setFontStyle('italic').setFontColor(COLORS.NEUTRAL_TEXT).setHorizontalAlignment('center').setBackground(COLORS.NEUTRAL_BG);
    row++;
  } else {
...
function refreshFilterScan() {
  ...
  if (!data || !data.stocks || !configOk) {
    sheet.getRange(row, 1, 1, NCOL).merge().setValue('⚠ API unreachable — /api/v8/raw or a filter_config returned no data').setBackground(COLORS.FAIL_BG).setFontColor(COLORS.FAIL_TEXT);
    return;
  }
  ...
  BASKETS.forEach((b, i) => { const meta = BASKET_META[b]; const startCol = 2 + i * 2; sheet.getRange(bandRow, startCol, 1, 2).merge().setValue(`${meta.emoji}  ${meta.label}`).setBackground(meta.color).setFontColor(COLORS.WHITE).setFontWeight('bold').setFontSize(10).setHorizontalAlignment('center'); });
  ...
  if (matrix.length === 0) {
    sheet.getRange(row, 1, 1, NCOL).merge().setValue('No stocks in universe for the latest score date').setFontStyle('italic').setFontColor(COLORS.NEUTRAL_TEXT).setHorizontalAlignment('center').setBackground(COLORS.NEUTRAL_BG);
    row++;
  } else {
    const dataRange = sheet.getRange(row, 1, matrix.length, NCOL);
    dataRange.setValues(matrix); dataRange.setFontFamily(FONTS.MONO.family).setFontSize(9).setVerticalAlignment('middle');
    sheet.getRange(row, 1, matrix.length, 1).setFontFamily(FONTS.HEADER.family).setFontWeight('bold').setHorizontalAlignment('left');
    BASKETS.forEach((b, i) => { const cntCol = 2 + i * 2; sheet.getRange(row, cntCol, matrix.length, 1).setHorizontalAlignment('center').setFontWeight('bold'); sheet.getRange(row, cntCol + 1, matrix.length, 1).setHorizontalAlignment('left').setFontColor(COLORS.NEUTRAL_TEXT); });
    for (let r = 0; r < matrix.length; r++) { sheet.getRange(row + r, 1, 1, NCOL).setBackground(r % 2 === 0 ? COLORS.CARD_BG : COLORS.ALT_ROW).setBorder(true, true, true, true, false, false, COLORS.BORDER_SOFT, SpreadsheetApp.BorderStyle.SOLID); }
    for (let r = 0; r < matrix.length; r++) {
      BASKETS.forEach((b, i) => {
        const cntCol = 2 + i * 2; const cellVal = matrix[r][cntCol - 1]; const parts = String(cellVal).split('/');
        if (parts.length === 2 && parts[0] === parts[1] && Number(parts[1]) > 0) { sheet.getRange(row + r, cntCol).setBackground(COLORS.PASS_BG).setFontColor(COLORS.PASS_TEXT).setFontWeight('bold'); }
      });
    }
    row += matrix.length;
  }
...
function renderTableHeader(sheet, row, headers, ncol) {
  headers.forEach((h, i) => { sheet.getRange(row, 1 + i).setValue(h).setFontWeight('bold').setBackground(COLORS.NEUTRAL_BG).setFontSize(10).setFontColor(COLORS.NEUTRAL_TEXT).setHorizontalAlignment('center').setBorder(true, true, true, true, false, false, COLORS.BORDER_SOFT, SpreadsheetApp.BorderStyle.SOLID); });
  return row + 1;
}
...
function humanLogic(metric) {
  const map = {
    'gvm_score': 'Quality gate', 'year_return': 'Long-term trend', 'dma_200': 'vs 200-day MA', 'dma_50': 'vs 50-day MA', 'dma_20': 'vs 20-day MA',
    'rsi_month': 'Monthly RSI', 'rsi_weekly': 'Weekly RSI', 'daily_rsi': 'Daily RSI', 'month_return': 'Monthly return', 'week_return': 'Weekly return',
    'sector_week': 'Sector week trend', 'sector_day': 'Sector today', 'month_index': 'Market breadth', 'week_index_52': '52-week position',
    'range_1d': "Today's candle", 'range_3d': '3-day move', 'ma9_vs_ma21': 'Short-term stretch', 'vol_ratio': 'Volume drying',
  };
