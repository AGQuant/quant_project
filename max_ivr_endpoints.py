...
STAGE MARKING. 13822 stages the work: stage 1 = the branch tree, stage 2 = a native card template
for every leaf, stage 3 = Gemini (DEFERRED — do not wire). Leaves whose intent does not exist yet
carry {"stage": 2} in action_params and render as an honest "building this week" state rather than
firing a query the router would not match. Nothing silently pretends to work.
...
#   stage     2 => template not built yet; the UI shows a building state instead of firing
...
_ANALYZE = [
    ...
    ("an_why",     "Why is it moving today?",        "why_moving",           4,  {"stage": 2}),
    ("an_chart",   "Chart",                          "card_C",               5,  {"card": "C"}),
    ("an_result",  "Result Card",                    "card_R",               6,  {"card": "R"}),
    ("an_levels",  "Levels · 52W & Pivots",          "levels",               7,  {"stage": 2}),
    ("an_fin",     "Financials Snapshot",            "fin_snapshot",         8,  {"stage": 2}),
    ("an_shp",     "Shareholding Trend",             "shareholding",         9,  {"stage": 2}),
    ("an_div",     "Dividend & Yield",               "dividend",             10, {"stage": 2}),
    ...
    ("an_news",    "Polished News",                  "polished_news_symbol", 14, {"stage": 2}),
]
...
_SIGNALS = [
    ...
    ("sg_pivots",  "Pivot Levels",            "paper_pivots",    8,  {"stage": 2}),
    ("sg_oi",      "OI Long / Short Buildup", "oi_buildup",      9,  {"stage": 2}),
    ("sg_rvol",    "Volume Spikes (RVOL)",    "rvol_spikes",     10, {"stage": 2}),
    ("sg_brk",     "52W-High Breakouts Today", "breakouts_today", 11, {"stage": 2}),
    ("sg_v10",     "Index Intel (V10)",       "v10_signal",      12, {"stage": 2}),
    ("sg_v9",      "Pairs (V9)",              "v9_pairs",        13, {"stage": 2}),
    ("sg_tc",      "Intraday Scan (TC)",      "tc_scan",         14, {"stage": 2}),
]

_MARKET = [
    ("mk_digest",  "Daily Digest",                        "daily_digest",          1, {"q": "daily digest"}),
    ("mk_mood",    "Market Mood",                         "v8_mood",               2, {"q": "market mood"}),
    ("mk_idxlvl",  "Nifty / BankNifty Support & Resistance", "index_levels",        3, {"stage": 2}),
    ("mk_pcr",     "ADR & PCR",                           "pcr",                   4, {"q": "pcr and adr today"}),
    ("mk_fii",     "FII / DII Flows",                     "fii_dii_positioning",   5, {"stage": 2}),
    ("mk_global",  "Global · Commodities · Currency",     "global_indices_card",   6, {"stage": 2}),
    ("mk_rtoday",  "Results Today",                       "results_today",         7, {"stage": 2}),
    ("mk_rweek",   "Results This Week",                   "results_week",          8, {"stage": 2}),
]
...
_SECTORS = [
    ...
    ("sc_rot",     "Sector Rotation",          "sector_rotation", 5, {"stage": 2}),
]

_SECTOR_L2 = [
    ("sc_members", "Members by GVM",       "sector_segment", 1, {"q": "stocks in {seg} by gvm"}),
    ("sc_picks",   "Top Picks",            "sector_segment", 2, {"q": "top stocks in {seg}"}),
    ("sc_move",    "Segment Move",         "sector_segment", 3, {"stage": 2}),
    ("sc_brief",   "Segment Brief",        "sector_brief",   4, {"stage": 2}),
]

_SCREENS = [
    ("sn_screens", "The predefined screens",  "screen_members",   1,  {"stage": 2}),
    ("sn_excel",   "Excellent (8+)",          "gvm_strong",       2,  {"q": "stocks with gvm above 8"}),
    ("sn_top10",   "Top 10 by GVM",           "gvm_top",          3,  {"q": "top 10 stocks by gvm"}),
    ("sn_div",     "High Dividend Yield",     "filter_dividend",  4,  {"stage": 2}),
    ("sn_debt",    "Debt-Free Quality",       "filter_debtfree",  5,  {"stage": 2}),
    ("sn_52w",     "Near 52W High",           "filter_near52w",   6,  {"stage": 2}),
    ("sn_osold",   "Oversold Bounce Candidates", "filter_oversold", 7, {"stage": 2}),
    ("sn_small",   "Small Caps by GVM",       "gvm_top_smallcap", 8,  {"stage": 2}),
    ("sn_own",     "Build your own",          "deeplink",         9,  {"href": "/v13"}),
]

_RESULTS = [
    ("rs_season",  "Season Summary",          "result_season",       1, {"stage": 2}),
    ("rs_sector",  "Sector Table",            "result_sector_table", 2, {"stage": 2}),
    ("rs_beats",   "Beats & Misses",          "result_beats",        3, {"stage": 2}),
    ("rs_today",   "Reported Today",          "results_today",       4, {"stage": 2}),
    ("rs_lookup",  "Look up a company",       "card_R",              5, {"card": "R", "needs": "symbol"}),
]

_MF = [
    ("mf_screen",  "Curated MF Screener",     "mf_screener",  1, {"stage": 2}),
    ("mf_mqs",     "Top Funds by MQS",        "mf_top_mqs",   2, {"stage": 2}),
    ("mf_mine",    "Is My Fund Good?",        "mf_fund",      3, {"stage": 2}),
    ("mf_large",   "Large Cap",               "mf_category",  4, {"stage": 2}),
    ("mf_mid",     "Mid Cap",                 "mf_category",  5, {"stage": 2}),
    ("mf_small",   "Small Cap",               "mf_category",  6, {"stage": 2}),
    ("mf_flexi",   "Flexi Cap",               "mf_category",  7, {"stage": 2}),
    ("mf_elss",    "ELSS / Tax Saver",        "mf_category",  8, {"stage": 2}),
]

_PORTFOLIO = [
    ("pf_check",   "Portfolio Health Check",  "hr_check",              1, {"href": "/health"}),
    ("pf_saved",   "Saved Reports",           "hr_reports_list",       2, {"href": "/adaptive"}),
    ("pf_exit",    "Exit Check a Holding",    "exit_check_composite",  3, {"stage": 2}),
    ("pf_qb",      "QB Summary",              "qb_summary",            4, {"q": "qb summary"}),
    ("pf_qbpos",   "QB Positions",            "qb_positions",          5, {"stage": 2}),
