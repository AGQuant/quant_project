...
(function () {
  ...
  async function qaAnalysis(sym){
    ...
    // above (PAGE_IDS.d = 'mDcBtn' on /dashboard, OWN_IDS.d = 'scorrAnaCard' on the self-built
    // shell). The old hardcoded 'mDcBtn' lookup was falsy on /m/* so #scorrAnaCard stayed empty and
    // its :empty rule hid the whole row. Same try/catch, same active letter, same order (after the
    // shell clear).
