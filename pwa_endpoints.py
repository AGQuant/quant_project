...
APP_THEME_RESOLVE_JS = """(function(){
  var ALLOWED={goldnight:1,dark:1,aquawhite:1,blush:1,ainight:1,silvergold:1,electricviolet:1,indigoash:1,duskday:1,orangepeel:1};   /* cc#1792: Electric Violet REPLACES Dusk Violet (39762) — duskviolet withdrawn from the list, its block stays, a stored duskviolet is remapped below */   /* cc#1791: + Orange Peel (light, THEME_ORANGE_PEEL_V1 39738) */   /* cc#1790: + Dusk Day (light, THEME_DUSK_DAY_V1 39734) */   /* cc#1775: + Dusk Violet, Indigo Ash (dark). cc#1776 (THEME_SWITCHER_FINAL_8_V1, 39651): Wine Purple WITHDRAWN from the list — its 21-key block stays in scorr_themes.css, dormant like goldday/rosenight/rosewall; a stored 'winepurple' is remapped below. */   /* cc#1719: + Silver Gold, Wine Purple. cc#1654 (founder 04-Sep ~10:00
  ...
  var REMAP={goldday:'aquawhite',rosenight:'blush',rosewall:'blush',winepurple:'electricviolet',duskviolet:'electricviolet'};   /* cc#1792: duskviolet -> electricviolet once, written back; winepurple now lands there too (its old target left the switcher) */   /* cc#1776: a phone left on Wine Purple lands on Dusk Violet ONCE, written back (remapped() below), so the switcher tick has a row to land on */   /* cc#1654: the dark->ainight
  ...
  var PREVIEW={aqua_white:'aquawhite',aquawhite:'aquawhite',gold_day:'goldday',goldday:'goldday',gold_night:'goldnight',goldnight:'goldnight',dark:'dark',blush:'blush',rose_night:'rosenight',rosenight:'rosenight',rose_wall:'rosewall',rosewall:'rosewall',ai_night:'ainight',ainight:'ainight',silver_gold:'silvergold',silvergold:'silvergold',wine_purple:'winepurple',winepurple:'winepurple',dusk_violet:'duskviolet',duskviolet:'duskviolet',indigo_ash:'indigoash',indigoash:'indigoash'};   /* ?theme= stays a one-load preview, never stored; cc#1719 adds the two new sets */
  function stored(){try{return localStorage.getItem('scorr_theme');}catch(e){return null;}}
  function remapped(t){
    if(t&&REMAP[t]){var r=REMAP[t];try{localStorage.setItem('scorr_theme',r);}catch(e){}return r;}
    return t;
  }
  function previewed(){
  ...
  var NAMES=[['goldnight','Gold Night'],['dark','Midnight Blue'],['ainight','AI Night'],['electricviolet','Electric Violet'],['indigoash','Indigo Ash'],['aquawhite','Aqua White'],['silvergold','Silver Gold'],['blush','Blush'],['duskday','Dusk Day'],['orangepeel','Orange Peel']];   /* cc#1792: Electric Violet in the Dusk Violet slot (dark group), still 10 */   /* cc#1791: 10 = 5 dark + 5 light; Orange Peel LAST in the light group */   /* cc#1790: 9 = 5 dark + 4 light; Dusk Day LAST in the light group; 39734 supersedes the 39651 "exactly 8" lock */   /* cc#1776: FINAL 8 — DARK Gold Night, Midnight Blue, AI Night, Dusk Violet, Indigo Ash; LIGHT Aqua White, Silver Gold, Blush (Blush last, cc#1750); Wine Purple withdrawn */   /* cc#1775: the two new DARK sets sit with the dark group after AI Night; Blush stays last per cc#1750 */   /* cc#1750 (founder 06-Sep): Blush last, after Wine Purple — order only */   /* cc#1654: Dark restored (founder 04-Sep); goldday/rosenight/rosewall stay hidden, order founder-set. cc#1719: Silver Gold + Wine Purple after Blush, visible for founder comparison. cc#1724: label Dark -> Midnight Blue (founder 05-Sep), key dark unchanged so saved picks survive */
