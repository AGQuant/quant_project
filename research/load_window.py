...
Robust, single-shot, chunked inserts so a slow proxy connection can't hang.
...
def main():
    ...
    print(f"got {len(cs)} candles — inserting in batches of {BATCH} ...", flush=True)
    ...
    print(f"DONE. DB rows now: {cur.fetchone()[0]}", flush=True)
    cur.close()
    conn.close()
