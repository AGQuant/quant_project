...
14. **CC_DEFAULT_BUILD_RULE_V1 (session_log id=33025, founder-set 28-Aug-2026; supersedes the ROLE_CHARTER_V4 Role Split default that BUILD OWNERSHIP follows the app-vs-web surface split, for UI/UX work only):** Arpit decides who pushes a UI/UX task based on its **complexity**, not automatically by surface. **Default: if a task does not explicitly name Fable or Claude AI as the pusher, CC completes it — app or web.** Fable/Claude AI push a UI/UX task only when Arpit **explicitly** assigns it to them for that specific task. Scope is UI/UX build-ownership only — it does NOT touch: the Hard Lines (Fable never pushes an engine/scheduler/`worker/**`/live-trading-path file), Fable's database-management ownership, the CC↔Fable audit-both-directions relationship, or Fable never editing `main.py`. Full statement + rationale: Role Split section below.
15. **PUSH_IS_NOT_DEPLOY_V1 (session_log id=44884, category=doctrine, founder-set 12-Sep-2026):** a push is **NOT done** until the commit is on **`main`** AND deployed. Landing on a feature branch is an intermediate state, never a finished one — **CC carries every push through to `main` in the same working session**, not left for a later merge. Origin: 12-Sep-2026, eight cards (cc#2002-cc#2009) sat on a feature branch with zero pull requests ever opened for it; Railway builds from `main`, so none of that work reached the app, unnoticed for hours because every per-push check CC ran (`git fetch`/`rev-parse`/`merge-base --is-ancestor`) came back clean — those prove a commit reached origin and the branch is intact, they do **NOT** prove it is on `main`, and may not be reported as landing evidence. **Required landing evidence:** the commit is reachable from `main`, and `github_read` of a changed file against `main` resolves. **Vocabulary:** COMMITTED means on `main` AND deployed — a commit sitting on a feature branch is not COMMITTED in the status-ladder sense; never write "pushed and verified" for branch-only work, since nothing has run in production and there is nothing to have verified. **Never squash** (Fable verifies against per-card SHAs; a squash destroys that mapping) **and never force-push `main`.** This does not override the gates that already outrank a merge — if any of them blocks it, CC STOPS, leaves the work on the branch, and says so in the task log; blocked-and-reported is correct, blocked-and-silent is exactly what this rule exists to prevent: the FEED WORKER DEPLOY RULE (cc#416, the Deploy policy section below — `worker/**` lands outside market hours or on a non-trading day unless the founder explicitly approves), MAINTENANCE_LOCK_RULE (cc#351, rule 10 above), and a genuine merge conflict (main having moved independently outranks the merge; STOP and report, never resolve unilaterally). `main` is the branch Railway's auto-deploy actually builds from — stated here so this is a known fact, not folklore.
...
## Archived / superseded rules (see rule 12's process above)

This section is the `archived_superseded` home the Supersession process refers to — the old text
stays here, dated and attributed, so nothing downstream keeps citing a rule that no longer applies.

- **28-Aug-2026, session_log id=33025 (CC_DEFAULT_BUILD_RULE_V1, rule 14):** superseded the
  ROLE_CHARTER_V4 Role Split's default that **build ownership follows the app-vs-web surface**
  (i.e. "APP/mobile UI is categorically Fable's to build, WEB is categorically CC's to build").
  Founder instruction, verbatim: *"I will decide fable push depends on complexity not all ui ux
  should be fable push, I will explicitly tell if fable or claude ai need to push otherwise cc has
  to complete if not mentioned explicitly."* Replaced by rule 14 above. Does not touch: Fable's
  database-management ownership, the Hard Lines (engine/scheduler/`worker/**`/live-trading-path —
  Fable never), the audit-both-directions relationship, or Fable never editing `main.py` — those
  are unchanged and still governed by ROLE_CHARTER_V4 below.

## Role Split — who owns what (ROLE_CHARTER_V4, 20-Aug-2026)

**UI/UX BUILD-OWNERSHIP DEFAULT HAS CHANGED — read this before the app-vs-web split below.**
**CC_DEFAULT_BUILD_RULE_V1 (session_log id=33025, founder-set 28-Aug-2026, rule 14)** supersedes
the app-vs-web split as the *automatic build-assignment rule* for UI/UX work. The domain
descriptions below still describe each seat's specialty and native ownership (Fable = app dev +
database; CC = web + backend), but **who actually pushes a given UI/UX task is now Arpit's call,
made per task based on complexity — not inferred from whether it's an app screen or a web page.**
**Default when a task does not explicitly name a pusher: CC builds it, app or web.** Fable /
Claude AI only push a UI/UX task when Arpit explicitly assigns it to them for that specific task.
This changes UI/UX build-ownership only — Fable's database-management ownership, the Hard Lines,
the audit-both-directions relationship, and Fable never touching `main.py` are all unchanged.
...
- APP / mobile UI: design refs, the build, and **direct pushes** of new files or whole pages Fable authored. **(Native domain — but per CC_DEFAULT_BUILD_RULE_V1 above, only builds when Arpit explicitly assigns the task to Fable; otherwise CC builds it by default.)**
...
- **Database management** — schema stewardship inside MAINTENANCE_LOCK_RULE, data quality, reconciles, DB-side operations. **(Unaffected by rule 14 — stays Fable's regardless.)**
- All DB writes: `session_log`, `cc_tasks`, registry and reference tables.

**CC — web + backend + audit**
- **WEB** surfaces (the desktop/web dashboards), built to Fable's specs. The surface split is APP = retail = Fable, WEB = premium = CC. **(Per CC_DEFAULT_BUILD_RULE_V1, CC is now also the UI/UX default builder for APP surfaces too, unless Arpit explicitly assigns a specific task to Fable.)**
...
- **A cc_task with no explicit pusher named is CC's by default (rule 14), whatever surface it touches.** A card is only Fable's to build when Arpit's brief or the card itself names Fable/Claude AI as the pusher.
...
- Rule 14 (CC_DEFAULT_BUILD_RULE_V1) does not touch any line on this list — it is scoped to UI/UX build-ownership only.
...
A push from either seat is a **claim** until the other seat's check passes. Fable verifies CC's pushes with the committed diff plus a DB query on real rows. CC audits Fable's pushes the same way, day-end. Neither seat marks its own work verified. **This is unchanged by rule 14 — whoever builds a task, the other seat still verifies it.**
...
- ~~App and mobile surfaces stay Fable's, per the Role Split above.~~ **SUPERSEDED 28-Aug-2026 by CC_DEFAULT_BUILD_RULE_V1 (session_log 33025, rule 14): app/mobile UI defaults to CC too now, same as web, unless Arpit explicitly assigns the specific task to Fable.** This bullet originally scoped founder-direct-to-CC to web only; that scope limit no longer applies — see rule 14.
...
## News polish (NEWS_POLISH_CANON_V5, session_log 45416)
...
**Batch shape (Fable's, for reference):** 12 items = 2 AI Editorial + 5 Domestic + 2 Global + 1 IPO + 2 Stock Views. The total is a ceiling, the split is a target, quality is the floor. Never pad to hit a number. Stock Views is merged into every news-polish batch now -- the 2 Stock Views items follow STOCK_VIEWS_FRAMEWORK_V2's own methodology (session_log 13456), read together with 45416 on a "news polish" trigger.
...
**Note (rule 14, 28-Aug-2026): this "APP card = Fable builds" lane now applies only when Arpit
explicitly assigns the task to Fable. An APP card with no explicit pusher named defaults to the
WEB/BACKEND lane below — CC builds it.**
