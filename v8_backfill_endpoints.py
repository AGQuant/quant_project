...
def backfill_metrics(x_admin_token: str = Header(None)):
    ...
    Uses 3yr raw_prices history (from 2023-06-01) for DMA200 warmup.
    """
    ...
    log.info(f"Symbols: {len(by_sym)}, BT days: {len(bt_dates)}")
    ...
    for dt in bt_dates:
        ...
        sector[dt] = {
            "sw": float(np.mean(wv)) if wv else None,
            "sm": float(np.mean(mv)) if mv else None,
        }

    # Batch upsert into v8_metrics
    inserted = 0
    with _conn() as conn, conn.cursor() as cur:
        for (sym, dt), m in all_metrics.items():
            ...
            try:
                ...
                log.warning(f"backfill insert {sym} {dt}: {e}")
        conn.commit()

    log.info(f"Backfill complete: {inserted} rows inserted")
    return {
        "status": "ok",
        "symbols": len(by_sym),
        "bt_days": len(bt_dates),
