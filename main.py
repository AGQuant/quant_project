...
def create_tables():
    try:
        ...
        cursor.execute("""CREATE TABLE IF NOT EXISTS sector_ratings (
            id SERIAL PRIMARY KEY,
            segment TEXT NOT NULL, stocks_count INT,
            mcap_weighted_gvm NUMERIC, weighted_g NUMERIC, weighted_v NUMERIC, weighted_m NUMERIC,
            simple_avg_gvm NUMERIC, total_mcap NUMERIC, top_stock TEXT, top_stock_gvm NUMERIC,
            verdict TEXT, score_date DATE NOT NULL, created_at TIMESTAMP DEFAULT NOW(),
            UNIQUE(segment, score_date)
        )""")
        cursor.execute("""CREATE TABLE IF NOT EXISTS momentum_scores (
            id SERIAL PRIMARY KEY,
            symbol TEXT NOT NULL, score_date DATE NOT NULL,
            latest_price NUMERIC, ret_1y NUMERIC, ret_3y NUMERIC,
            dma_50 NUMERIC, dma_200 NUMERIC, ret_52w_vs_index NUMERIC,
            ret_1y_rating NUMERIC, ret_3y_rating NUMERIC, dma_50_rating NUMERIC,
            dma_200_rating NUMERIC, ret_52w_idx_rating NUMERIC, m_score NUMERIC,
            created_at TIMESTAMP DEFAULT NOW(),
            UNIQUE(symbol, score_date)
        )""")
        cursor.execute("""CREATE TABLE IF NOT EXISTS v5_signals (
            id SERIAL PRIMARY KEY,
            signal_type TEXT NOT NULL, timestamp TEXT, symbol TEXT NOT NULL,
            ...
            event_date TEXT, event_type TEXT, verdict_date TEXT, notes TEXT,
            loaded_at TIMESTAMP DEFAULT NOW()
        )""")
        cursor.execute("""CREATE TABLE IF NOT EXISTS v5_futures_open (
            id SERIAL PRIMARY KEY, symbol TEXT NOT NULL, open_date TEXT, type TEXT,
            qty NUMERIC, entry_price NUMERIC, current_price NUMERIC,
            net_pl_pct NUMERIC, profit_per_lot NUMERIC, value NUMERIC,
            remarks TEXT, status TEXT, exported_at TIMESTAMP DEFAULT NOW()
        )""")
        cursor.execute("""CREATE TABLE IF NOT EXISTS v5_trades (
            id SERIAL PRIMARY KEY, symbol TEXT NOT NULL, open_date TEXT, type TEXT,
            future_price NUMERIC, target_future NUMERIC, stoploss_future NUMERIC,
            status TEXT, profit_per_lot NUMERIC, exported_at TIMESTAMP DEFAULT NOW()
        )""")
        cursor.execute("""CREATE TABLE IF NOT EXISTS v5_portfolio (
            id SERIAL PRIMARY KEY, symbol TEXT NOT NULL, portfolio_name TEXT,
            date_of_pick TEXT, buy_price NUMERIC, current_price NUMERIC,
            return_pct NUMERIC, qty NUMERIC, buy_value NUMERIC, market_value NUMERIC,
            weight_pct NUMERIC, mcap_category TEXT, finkhoz_rating NUMERIC,
            sector TEXT, upcoming_result TEXT, exported_at TIMESTAMP DEFAULT NOW()
        )""")
        cursor.execute("""CREATE TABLE IF NOT EXISTS earnings_calendar (
            id SERIAL PRIMARY KEY, company_name TEXT, ticker TEXT NOT NULL,
...
async def register_client(request: Request):
    ...
    return {"client_id": f"claude-{secrets.token_hex(8)}", "redirect_uris": body.get("redirect_uris", []),
            "grant_types": ["authorization_code"], "response_types": ["code"], "token_endpoint_auth_method": "none"}

@app.get("/authorize")
async def authorize(response_type: str = "code", client_id: str = "", redirect_uri: str = "",
...
# FIX 1: SG tab has symbol in col index 1 (leading blank col), header detected by 'Open Date'+'Entry Price'
def _parse_futures_open(df):
    ...
    for _, row in df.iterrows():
        # Symbol is col index 1 (col 0 is blank in SG tab)
        sym = _v5_clean(str(row.iloc[1]) if len(row) > 1 else '')
...
# FIX 2: Buy/Sell Reversal/Momentum tabs are screener matrices — extract NSE symbols
...
async def load_input_from_drive(request: Request):
    try:
        ...
        cur.execute("""CREATE TABLE IF NOT EXISTS input_raw (
            id SERIAL PRIMARY KEY, nse_code TEXT, company_name TEXT, bse_code TEXT, cmot_code TEXT,
        ...
        inserted = 0
        for _, row in df.iterrows():
            cur.execute("""INSERT INTO input_raw (nse_code,company_name,bse_code,cmot_code,
                ...
                num(row.get('Market Capitalization')), clean(row.get('Overview')),
                clean(row.get('Key Takeways')), clean(row.get('Segment')),
...
async def load_screener_from_drive(request: Request):
    try:
        ...
        for c in df.columns:
            if c not in TEXT_COLS: df[c] = pd.to_numeric(df[c], errors='coerce')
        for c in TEXT_COLS:
            if c in df.columns: df[c] = df[c].astype(str).replace({"nan": None, "None": None, "": None})
        ...
        cur.execute(f"CREATE TABLE screener_raw (id SERIAL PRIMARY KEY, {cols_def}, loaded_at TIMESTAMP DEFAULT NOW())")
        conn.commit()
        cols = list(df.columns)
        insert_sql = f"INSERT INTO screener_raw ({', '.join(f'{chr(34)}{c}{chr(34)}' for c in cols)}) VALUES ({', '.join(['%s']*len(cols))})"
        inserted = 0; batch = []
        for _, row in df.iterrows():
            vals = []
            for c in cols:
                ...
                try:
                    if v is None: vals.append(None)
                        ...
                        s = str(v).strip(); vals.append(s if s and s.lower() != "nan" else None)
                    elif pd.isna(v): vals.append(None)
                    else: vals.append(float(v))
                except: vals.append(None)
            batch.append(vals)
            if len(batch) == 100:
                cur.executemany(insert_sql, batch); conn.commit(); inserted += len(batch); batch = []
        if batch:
...
async def load_v5_from_drive(request: Request):
    ...
    if not file_ids: return {"error": "file_ids required"}

    # FIX 3: Buy/Sell tabs use _parse_v5_screener instead of _parse_signal_tab
    FILE_MAP = {
    ...
    for filename, file_id in file_ids.items():
        ...
        try:
            ...
            conn.rollback(); results[filename] = f"error: {str(e)}"
    cur.close(); conn.close()
    ...
    try:
        ...
        cur.execute("""
            ...
            SELECT s.segment, s.stocks, s.w_gvm, s.w_g, s.w_v, s.w_m,
                   ...
                   CASE WHEN s.w_gvm >= 7.0 THEN 'Strong' WHEN s.w_gvm >= 6.5 THEN 'Buy'
                        WHEN s.w_gvm >= 6.0 THEN 'Watch' ELSE 'Avoid' END, s.score_date
            FROM (
                SELECT segment, COUNT(*) AS stocks,
                       ...
                       SUM(market_cap) AS total_mcap, MAX(score_date) AS score_date
                FROM gvm_scores
                WHERE score_date=(SELECT MAX(score_date) FROM gvm_scores) AND market_cap IS NOT NULL
...
async def refresh_momentum():
    try:
        ...
        cur.execute("""
            ...
            latest_px AS (SELECT symbol, close AS latest_close FROM raw_prices
                WHERE price_date=(SELECT dt FROM d) AND symbol NOT IN ('NIFTY50','BANKNIFTY')),
            px_1y AS (SELECT DISTINCT ON (symbol) symbol, close AS close_1y FROM raw_prices
                WHERE price_date<=(SELECT dt FROM d)-INTERVAL '365 days'
                AND symbol NOT IN ('NIFTY50','BANKNIFTY') ORDER BY symbol, price_date DESC),
            px_3y AS (SELECT DISTINCT ON (symbol) symbol, close AS close_3y FROM raw_prices
                WHERE price_date<=(SELECT dt FROM d)-INTERVAL '1095 days'
                AND symbol NOT IN ('NIFTY50','BANKNIFTY') ORDER BY symbol, price_date DESC),
            dma50 AS (SELECT symbol, AVG(close) AS dma50_val
            ...
            dma200 AS (SELECT symbol, AVG(close) AS dma200_val
                FROM (SELECT symbol, close, ROW_NUMBER() OVER (PARTITION BY symbol ORDER BY price_date DESC) AS rn
                      FROM raw_prices WHERE symbol NOT IN ('NIFTY50','BANKNIFTY')) x
            ...
            FROM latest_px lp JOIN px_1y p1 USING (symbol)
        ...
        """)
        cur.execute("""UPDATE momentum_scores SET
...
async def _call_tool(name: str, args: dict) -> dict:
    async with httpx.AsyncClient(timeout=120) as client:
        if name == "get_gvm":
            r = await client.get(f"{BASE_URL}/api/gvm/{args.get('symbol', '')}"); return r.json()
        elif name == "get_top_stocks":
            p = {"n": args.get("n", 20)}
            if args.get("verdict"): p["verdict"] = args["verdict"]
            r = await client.get(f"{BASE_URL}/api/gvm/top", params=p); return r.json()
        elif name == "get_sector":
            r = await client.get(f"{BASE_URL}/api/gvm/sector",
                params={"segment": args.get("segment", ""), "n": args.get("n", 20)}); return r.json()
        elif name == "get_filter":
            p = {"min_score": args.get("min_score", 7.0), "max_score": args.get("max_score", 10.0), "n": args.get("n", 50)}
            if args.get("verdict"): p["verdict"] = args["verdict"]
            r = await client.get(f"{BASE_URL}/api/gvm/filter", params=p); return r.json()
        elif name == "run_sql":
            query = args.get("query", ""); params = args.get("params", [])
                ...
                with psycopg.connect(db_url, row_factory=dict_row) as conn:
                    with conn.cursor() as cur:
                        ...
                        if cur.description is None:
                            conn.commit(); return {"status": "ok", "message": "Query executed."}
                        rows = cur.fetchall()
            ...
            if not file_id: return {"error": "file_id required"}
            r = await client.post(f"{BASE_URL}/api/admin/load_input_from_drive", json={"file_id": file_id}); return r.json()
        elif name == "load_screener_from_drive":
            file_id = args.get("file_id", "")
            if not file_id: return {"error": "file_id required"}
            r = await client.post(f"{BASE_URL}/api/admin/load_screener_from_drive", json={"file_id": file_id}); return r.json()
            ...
            stype = args.get("signal_type", ""); verdict = args.get("verdict", ""); n = int(args.get("n", 30))
            from psycopg.rows import dict_row
            db_url = os.environ.get("DATABASE_URL", "")
            with psycopg.connect(db_url, row_factory=dict_row) as conn:
                conds, params = [], []
                ...
                q += " ORDER BY loaded_at DESC, timestamp DESC LIMIT %s"; params.append(n)
                with conn.cursor() as cur:
            ...
            with psycopg.connect(db_url, row_factory=dict_row) as conn:
                with conn.cursor() as cur:
                    cur.execute("""SELECT ticker, event_type, ex_date::TEXT, record_date::TEXT
                        FROM earnings_calendar WHERE ticker = %s AND ex_date IN (%s, %s)""",
            ...
            with psycopg.connect(db_url, row_factory=dict_row) as conn:
                with conn.cursor() as cur:
                    cur.execute("SELECT * FROM v5_portfolio ORDER BY symbol"); rows = cur.fetchall()
            return {"rows": len(rows), "data": [dict(r) for r in rows]}
...
MCP_TOOLS = [
    ...
    {"name": "load_screener_from_drive", "description": "Reload screener_raw from Google Drive file. Pass file_id.",
     "inputSchema": {"type": "object", "properties": {"file_id": {"type": "string"}}, "required": ["file_id"]}},
    {"name": "get_sector_rating", "description": "Get mcap-weighted GVM rating for sectors. Pass segment for one, omit for all.",
     "inputSchema": {"type": "object", "properties": {"segment": {"type": "string"}, "n": {"type": "integer", "default": 20}}}},
    {"name": "get_momentum", "description": "Get momentum scores (1Y/3Y return, DMA50, DMA200, vs Nifty). Pass symbol for one stock.",
    ...
    {"name": "get_v5_signals", "description": "Get V5 alert signals. signal_type: Large_Cap, Mid_Cap, Small_Cap, Buy_Reversal, Buy_Momentum, Sell_Reversal, Sell_Momentum.",
     "inputSchema": {"type": "object", "properties": {"signal_type": {"type": "string"}, "verdict": {"type": "string"}, "n": {"type": "integer", "default": 30}}}},
    {"name": "check_blackout", "description": "Check if a stock has earnings today or tomorrow. Returns in_blackout: true/false.",
     "inputSchema": {"type": "object", "properties": {"symbol": {"type": "string"}}, "required": ["symbol"]}},
    {"name": "get_v5_portfolio", "description": "Get current V5 live portfolio holdings.",
     "inputSchema": {"type": "object", "properties": {}}},
]
...
async def mcp_handler(request: Request):
    try: body = await request.json()
    except: return JSONResponse({"jsonrpc": "2.0", "id": None, "error": {"code": -32700, "message": "Parse error"}})
    method = body.get("method", ""); params = body.get("params", {}); req_id = body.get("id")
    if req_id is None and method.startswith("notifications/"): return Response(status_code=202)
    if method == "initialize":
        return JSONResponse(content={"jsonrpc": "2.0", "id": req_id, "result": {
            "protocolVersion": "2024-11-05", "capabilities": {"tools": {}},
            "serverInfo": {"name": "Scorr — Project Quant", "version": "1.2.1"}}},
            headers={"Mcp-Session-Id": str(uuid.uuid4())})
    elif method == "tools/list":
        return {"jsonrpc": "2.0", "id": req_id, "result": {"tools": MCP_TOOLS}}
    elif method == "tools/call":
        tool_name = params.get("name"); tool_args = params.get("arguments", {})
        try:
...
class ParamRequest(BaseModel):
    stock_val: float; peer_avg: float
...
@app.get("/")
def root():
    return {"message": "Project Quant — Trading API is live 🚀", "version": "1.2.1", "mcp": "/mcp", "docs": "/docs"}
...
def get_top_stocks(n: int = 20, verdict: str = None):
    try:
        conn = get_db_conn(); cur = conn.cursor()
        cur.execute("SELECT MAX(score_date) FROM gvm_scores"); latest = cur.fetchone()[0]
        if verdict:
            cur.execute("""SELECT symbol,company_name,segment,rank,price,g_score,v_score,m_score,gvm_score,verdict
                FROM gvm_scores WHERE score_date=%s AND verdict=%s ORDER BY gvm_score DESC LIMIT %s""", (latest,verdict,n))
        else:
            cur.execute("""SELECT symbol,company_name,segment,rank,price,g_score,v_score,m_score,gvm_score,verdict
...
def get_by_sector(segment: str, n: int = 20):
    try:
        ...
        cur.execute("SELECT MAX(score_date) FROM gvm_scores"); latest = cur.fetchone()[0]
        cur.execute("""SELECT symbol,company_name,segment,rank,price,g_score,v_score,m_score,gvm_score,verdict
            FROM gvm_scores WHERE score_date=%s AND segment ILIKE %s ORDER BY gvm_score DESC LIMIT %s""",
            (latest,f"%{segment}%",n))
        rows = cur.fetchall(); cur.close(); conn.close()
...
def sector_rating_all(n: int = 200):
    try:
        conn = get_db_conn(); cur = conn.cursor()
        cur.execute("""SELECT segment,stocks_count,CAST(mcap_weighted_gvm AS FLOAT),
...
def momentum_top(n: int = 20, min_score: float = 7.0):
    try:
        conn = get_db_conn(); cur = conn.cursor()
        cur.execute("""SELECT m.symbol,CAST(m.latest_price AS FLOAT),CAST(m.ret_1y AS FLOAT),
                   CAST(m.ret_3y AS FLOAT),CAST(m.dma_50 AS FLOAT),CAST(m.dma_200 AS FLOAT),
            ...
            LEFT JOIN gvm_scores g ON g.symbol=m.symbol AND g.score_date=(SELECT MAX(score_date) FROM gvm_scores)
            WHERE m.score_date=(SELECT MAX(score_date) FROM momentum_scores) AND m.m_score>=%s
            ORDER BY m.m_score DESC,m.ret_1y DESC LIMIT %s""", (min_score,n))
        rows = cur.fetchall(); cur.close(); conn.close()
        return {"count":len(rows),"stocks":[
            {"symbol":r[0],"price":r[1],"ret_1y":r[2],"ret_3y":r[3],"dma_50":r[4],
             "dma_200":r[5],"vs_nifty":r[6],"m_score":r[7],"segment":r[8],"gvm_score":r[9],"verdict":r[10]}
            for r in rows]}
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
def momentum_for_symbol(symbol: str):
    try:
        conn = get_db_conn(); cur = conn.cursor()
        cur.execute("""SELECT symbol,CAST(latest_price AS FLOAT),CAST(ret_1y AS FLOAT),CAST(ret_3y AS FLOAT),
                   CAST(dma_50 AS FLOAT),CAST(dma_200 AS FLOAT),CAST(ret_52w_vs_index AS FLOAT),
                   CAST(ret_1y_rating AS FLOAT),CAST(ret_3y_rating AS FLOAT),CAST(dma_50_rating AS FLOAT),
                   CAST(dma_200_rating AS FLOAT),CAST(ret_52w_idx_rating AS FLOAT),CAST(m_score AS FLOAT),score_date::TEXT
            FROM momentum_scores WHERE symbol=%s AND score_date=(SELECT MAX(score_date) FROM momentum_scores)""",
            (symbol.upper(),))
        r = cur.fetchone(); cur.close(); conn.close()
        if not r: raise HTTPException(status_code=404, detail=f"{symbol} not in momentum_scores")
        return {"symbol":r[0],"price":r[1],"ret_1y":r[2],"ret_3y":r[3],"dma_50":r[4],"dma_200":r[5],
                "vs_nifty":r[6],"rating_1y":r[7],"rating_3y":r[8],"rating_dma50":r[9],
                "rating_dma200":r[10],"rating_vs_idx":r[11],"m_score":r[12],"score_date":r[13]}
    except HTTPException: raise
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
def health_feeds():
    try:
        today = date.today(); conn = get_db_conn(); cur = conn.cursor()
        cur.execute("""
        ...
        for r in rows:
            ...
            feeds.append({"source":source,"latest":latest,"records":records,"freshness":freshness,"days_old":days})
...
def run_daily_update():
    ...
    except Exception as e: logging.error(f"[SCHEDULER] Yahoo failed: {e}")
    try:
    ...
    except Exception as e: logging.error(f"[SCHEDULER] Auto-refresh failed: {e}")
