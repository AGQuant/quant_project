...
from qb_universe_derived import router as qb_universe_derived_router  # cc#2148: QB Universe night-window precompute (status + admin run)
...
from gvm_history_pit_backfill import router as gvm_pit_backfill_router   # cc#2092: point-in-time GVM history backfill (gated startup trigger)
...
# cc#1900: /m/sector had the SAME pre-existing gap as /m/screeners above — a working route
# (mobile_ext.py:635) in NONE of PROTECTED/NAV_REGISTRY/NAV array. Filed as "grid-tile" for the
# SAME reason (home.html's own gtile('/m/sector', ..., 'Sector Intel') is its only entry point,
...
app.include_router(qb_universe_derived_router)  # cc#2148: QB Universe derived precompute (status + admin run)
...
from chart_oneday_endpoints import router as chart_oneday_router   # cc#2043 UNIVERSAL RULE: GET /api/chart/oneday/{symbol} -- the one canonical Yahoo-only 1-day-chart fetch, callable app-wide, never Fyers, never a DB write
app.include_router(chart_oneday_router)
...
app.include_router(gvm_pit_backfill_router)   # cc#2092
