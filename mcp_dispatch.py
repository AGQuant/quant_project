...
MCP_TOOLS = [
    ...
    {"name":"tc_replay_run","description":"cc#1220: fire the cc#1211 TC score entry replay — five sessions, all four buckets, score100 at every 15-min tick, then the threshold x hold sweep. ASYNC: starts a background daemon and returns {started:true,phase,run_id} immediately, or {busy:true} if one is already walking. Roughly 100k card evaluations, so poll tc_replay_status rather than waiting. Safe to fire twice: scoring upserts on (ts,symbol,bucket) and the sweep clears each cell before refilling it. phase = all (default) | score | sweep | portfolio. cc#1221 PORTFOLIO mode re-walks the ALREADY-STORED ticks under a capped book - score100 >= 80 entry, candidates ranked by score and filled into at most 20 open positions across all four buckets, one per symbol, no same-day re-entry, +/-2% or a 3-session time exit. It never re-scores, so it is fast, and it is deliberately NOT part of `all`. This exists because POST /api/admin/run-tc-replay is unreachable — scorr.in is egress-blocked from the CC and Fable seats.","inputSchema":{"type":"object","properties":{"phase":{"type":"string","description":"all (default), score, or sweep"}},"required":[]}},
