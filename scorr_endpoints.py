...
def get_top_stocks_native(limit: int = 20, segment: str = None) -> list:
    try:
        with get_conn() as conn, conn.cursor() as cur:
            if segment:
                cur.execute(
                    "SELECT symbol, gvm_score, growth, value, momentum, segment FROM gvm_cache WHERE segment = %s AND gvm_score > 0 ORDER BY gvm_score DESC LIMIT %s",
                    (segment, limit)
                )
            else:
                cur.execute(
                    "SELECT symbol, gvm_score, growth, value, momentum, segment FROM gvm_cache WHERE gvm_score > 0 ORDER BY gvm_score DESC LIMIT %s",
                    (limit,)
                )
...
def filter_by_threshold_native(segment: str, threshold: float) -> list:
    try:
        with get_conn() as conn, conn.cursor() as cur:
            cur.execute(
                "SELECT symbol, gvm_score, growth, value, momentum, segment FROM gvm_cache WHERE segment = %s AND gvm_score >= %s ORDER BY gvm_score DESC",
                (segment, threshold)
            )
            cols = [d[0] for d in cur.description]
            return [dict(zip(cols, row)) for row in cur.fetchall()]
...
def get_peer_comparison_native(symbol: str, segment: str) -> dict:
    try:
        with get_conn() as conn, conn.cursor() as cur:
            ...
            return {
                "symbol": symbol, "gvm_score": score, "peer_avg": peer_avg,
...
async def scorr_query(req: ScorrQueryRequest):
    ...
    tokens_used = 0; api_calls = 0; result = None
    ...
    if req.type == "top_stocks":
        ...
        with get_conn() as conn, conn.cursor() as cur:
            for symbol in req.stocks:
                ...
                row = cur.fetchone()
                if row:
                    scores.append({"symbol": row[0], "gvm_score": float(row[1]), "growth": float(row[2]), "value": float(row[3]), "momentum": float(row[4]), "segment": row[5]})
                else:
...
# ── SmartGain M2M (cc_task #93) ─────────────────────────────────────────────

@router.get("/api/smartgain/m2m")
def smartgain_m2m():
    """SmartGain MHK40 holdings simulator — live M2M. Updated from screenshots."""
    try:
        with get_conn() as conn, conn.cursor() as cur:
            cur.execute("""
                SELECT symbol, direction, qty,
                       ROUND(entry_price::numeric, 2) AS entry_price,
                       ROUND(ltp::numeric, 2) AS ltp,
                       ROUND(mtm::numeric, 2) AS mtm,
                       updated_at
